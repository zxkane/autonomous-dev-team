...
# AND the [INV-102] convergence circuit-breaker (#297) share ONE source of truth
# for "is it safe to take a terminal action against this issue right now" — no
# copy-pasted `pid_alive` block ([INV-102] C4′/C9b).
#
...
mark_stalled() {
  ...
  #
  # [INV-102] (#297): the liveness predicate itself (pid_alive + local
...
# ---------------------------------------------------------------------------
# INV-102 (#297): convergence circuit-breaker helpers.
# ---------------------------------------------------------------------------
...
# `classify_recent_review_verdict` OUT-VARS — NOT body text ([INV-102] C1/C2). This
...
# length) and the [INV-102] report's per-round timestamp evidence ([P1] finding 2).
#
# Derivation ([INV-102] C1/C9a): rounds are the pre-existing per-round dispatcher
...
# `<frozen_head>` — the length of `_frozen_convergence_rounds_json` ([INV-102]
...
# the [INV-102] convergence report's evidence block. Mirrors
...
handle_completed_session_routing() {
  ...
  case "$_verdict" in
    ...
    failed-substantive)
      ...
      # [INV-102] (#297): the PR number for the convergence report's evidence line.
      local _np_pr_number
      ...
      # Branch B″ — [INV-102] (#297) convergence circuit-breaker. Reached ONLY for
      ...
      if [ "$_cb_threshold" -gt 0 ] && [ "$_cb_rounds" -ge "$_cb_threshold" ]; then
        ...
        if ! may_stall_now "$issue_num"; then
          log "  issue #${issue_num} convergence breaker: ≥${_cb_threshold} frozen-head rounds on \`${_np_current_head}\` but a dev wrapper is ALIVE — deferring terminal action ([INV-102])"
          return 0
        fi
        ...
        if [ "${_cb_present:-0}" != "0" ]; then
          log "  issue #${issue_num} convergence breaker already reported for head \`${_np_current_head}\` (trailer=${_cb_hash}) — idempotent no-op ([INV-102])"
          return 0
        fi

        log "  issue #${issue_num} NON-CONVERGENCE detected: ${_cb_rounds} completed zero-commit rounds on frozen head \`${_np_current_head}\` (trailer=${_cb_hash}) — halting per [INV-102]"
...
${_cb_marker}
## ⛔ Convergence circuit-breaker tripped — halting a non-converging dev↔review loop (\`reason=non-convergence\`, [INV-102])
...
- Transitioned the issue to \`stalled\` (autonomy halted; \`pending-dev\` removed) — re-adding \`autonomous\` is now the operator's explicit opt-in to resume.
...
**To resume: fix per the checklist above, then re-add the \`autonomous\` label.**
@${REPO_OWNER}
CBREPORT
)"
        label_swap "$issue_num" "pending-dev" "stalled"
