    ...
    failed-substantive)
      ...
      if [ "$_dev_actionable" = "false" ]; then
        ...
        if itp_list_comments "$issue_num" 2>/dev/null \
            ...
            2>/dev/null | grep -q '^0$'; then
          # INV-135 (#488) D4: surface the matched REVIEW_PROTECTED_PATHS
...
# INV-135 (#488) D4: echoes the newest `inv92-matched-patterns:` marker's
...
_same_head_verdict_aware_recovery() {
  ...
  if [ "$_dev_actionable" = "false" ]; then
    ...
    if itp_list_comments "$issue_num" 2>/dev/null \
        | jq -r "[.[].body | select(contains(\"${_na_marker}\"))] | length" \
        2>/dev/null | grep -q '^0$'; then
      # INV-135 (#488) D4: same marker-surfacing fallback as Branch B′ above,
