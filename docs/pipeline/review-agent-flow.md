...
**Scored-text source, per resolution path ([INV-132], issue #481)**: the call site selects the text by RESOLUTION CHANNEL, not by agent identity — `AGENT_VERDICT_BODIES[i]`'s rendered body (the artifact-rendered body for an artifact-sourced agent, the comment-fallback body otherwise) is preferred WHENEVER it is non-empty, for every agent including codex. `AGENT_CODEX_LOGS[i]`'s raw stdout is consulted ONLY when `AGENT_VERDICT_BODIES[i]` is empty (a codex agent resolved purely via the legacy stdout-classify path, no rendered body at all) — and even then the echoed prompt is stripped first (`_codex_review_strip_prompt_echo`, `adapters/codex.sh`) before scoring. Pre-#481, the codex branch was selected unconditionally by agent name, so an artifact-resolved codex verdict was ALWAYS scored from its raw stdout — which always echoes `build_review_prompt`'s own numbered checklist ahead of the agent's real output — collapsing the per-finding fail-safe scan (below) to `none` regardless of the agent's actual findings, which in turn falsely advanced [INV-127]'s round-cap counter on P2-only rounds.

The echo also has to be stripped where the body is COMPOSED, not only where the severity loop reads it: the wrapper's own stdout-derived verdict post ([INV-62], `_codex_review_compose_body`) populates `AGENT_VERDICT_BODIES[i]` for a codex `fail` BEFORE the severity loop ever runs, so a body composed from the raw, un-stripped stdout would still poison the scan via the PRIMARY (body-preferred) branch above — the severity loop's own stripping fallback never gets a chance to run on that live path (an empty-body codex `fail` doesn't occur; it resolves `unavailable` instead). `_codex_review_compose_body`'s FAIL branch therefore calls `_codex_review_strip_prompt_echo` itself before composing, so the body handed to every downstream consumer — the severity scan AND the human-facing GitHub comment — is already clean. See [INV-132](invariants.md#inv-132-the-pre-aggregation-severity-filter-always-scores-a-rendered-verdict-body-when-one-exists--the-raw-per-cli-stdout-capture-is-a-fallback-only-for-a-resolution-path-that-produced-no-body-at-all) for the full incident writeup.
