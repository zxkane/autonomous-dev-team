...
**The body-file path is namespaced per (agent, issue, session) — `#353`.** `build_review_prompt` renders `<body-file>` as `/tmp/verdict-${_agent_name}-${ISSUE_NUMBER}-${_agent_session_id}.md`, not the bare `/tmp/verdict-${_agent_name}.md` the prompt used before. The bare form is keyed on agent name alone, which is unique only *within one fan-out* — but `/tmp` is shared by every concurrent review on the host, across every project and issue. Two overlapping reviews (e.g. two projects both running a `codex` agent at once) raced on that same path: the later writer's body could land between an earlier agent's write and its `post-verdict.sh` read, so the earlier issue's verdict comment carried the *later* review's findings under the earlier issue's own (valid) `Review Session:` trailer — passing every [INV-20](invariants.md#inv-20-verdict-authenticity-binding-actor--window--trailer-presence)/[INV-40](invariants.md#inv-40-multi-agent-review-attribution-unanimous-aggregation-and-all-unavailable-fallback) attribution check. Namespacing by issue number + the agent's own session id makes the path collision-free across hosts-wide concurrency, mirroring the per-run uniqueness the [INV-78](invariants.md#inv-78-review-verdicts-resolve-from-a-typed-artifact-file-first-comment-scraping-is-an-explicitly-logged-fallback-a-malformed-artifact-is-loud-never-a-silent-absent) typed-artifact channel already had. The two wrapper-owned body files (the codex-stdout-fallback post and the aggregate-comment post, both further down this doc) were never affected — both already use `mktemp` with a random suffix.
