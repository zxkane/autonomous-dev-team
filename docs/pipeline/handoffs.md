...
### Machine markers used within H4/H5 (review round-tracking, issues #449 and #525)
...
| `<!-- dispatcher-review-unavailable-breaker: issue=<N> head=<sha\|unknown> round=<n> -->` | review wrapper, every `all-unavailable` round and every decided reset | head-AGNOSTIC — accumulates across consecutive zero-decision rounds; `head` is forensic-only. Every decided PASS or FAIL posts `round=0`. A trip posts the threshold marker followed by an independent reset before its report; the latest `stalled` removal event is an additional cutoff, so operator re-arm survives failure of every trip-side comment post ([INV-144](invariants.md#inv-144-consecutive-all-unavailable-review-aggregates-are-bounded-by-an-independent-durable-breaker-that-transitions-reviewing---stalled-and-posts-one-reasonreview-unavailable-cap-report)) | INV-144's own trip check (`_review_unavailable_next_count`/`_review_unavailable_threshold`, `lib-review-unavailable-cap.sh`) |
...
All four use an **unbounded** full-comment-history scan (`itp_list_comments`, no pagination cap). The three older marker readers filter to `authorKind != "human"`; INV-144 requests the provider's stricter exact-self mode (`ITP_REQUIRE_SELF_AUTHOR=1`), requires `authorKind=self`, and binds the marker's `issue=` field to the active issue, so unrelated Apps/bots cannot advance, reset, or cut off its streak. All four counters are posted on EVERY qualifying round, not only on a trip — a marker that only appears when the breaker fires would never let the SECOND occurrence find the FIRST one's count, and the counter could never advance past 1. "Qualifying round" for `review-round-counter` and `dispatcher-review-cap-breaker` ALSO requires `_aggregate_has_substantive_fail` (`lib-review-aggregate.sh`, [P1] codex review round 4) — a `fail` aggregate produced entirely by [INV-48] timeout vetoes (no agent posted findings text) does not post either marker, since it carries no evidence a real blocking finding exists. `review-round-counter` also posts `round=0` on PASS and every `failed-non-substantive` exit ([INV-129]); `dispatcher-review-unavailable-breaker` posts `round=0` on every decided PASS/FAIL and immediately after a trip ([INV-144]). The INV-127 and INV-122 markers have no explicit reset value of their own.
