...
|---|---|---|---|
| `<!-- review-round-counter: issue=<N> head=<sha> round=<n> -->` | review wrapper, every round | `(issue, head)` — a new HEAD resets `round` to 1; increments only while HEAD is unchanged | the severity-aware blocking ratchet's prompt wording + the pre-aggregation severity filter (both `lib-review-severity.sh`) — see [review-agent-flow.md § Prompt construction](review-agent-flow.md#prompt-construction) |
...
- A post failure (`itp_post_comment` non-zero) is swallowed (`|| true`) for BOTH the marker post and the report post independently — the tier decision and any label transition already happened; only the audit trail is missing for this tick.

**Consumer-side invariants** (the *next* tick's `_liveness_evaluate_issue`):

- Reads back via `_liveness_prior_marker` (`lib-liveness.sh`), which calls `itp_list_comments`'s result through a CUTOFF-then-scan (mirrors `_review_cap_prior_marker`, [INV-127](invariants.md#inv-127-a-review-side-divergent-findings-non-convergence-review_convergence_cap-consecutive-failed-substantive-rounds-where-the-severity-ratchet-s-own-p0p1-floor-is-still-failing-is-detected-and-halted--the-breaker-transitions-reviewing--stalled-then-posts-one-structured-reasonreview-round-cap-report-gated-on-an-already-stalled-skip-does-not-gate-on-may_stall_now-mirroring-inv-122s-own-rationale)): filters to comments STRUCTURALLY anchored on the marker's own exact grammar — **[round 6]** a WHOLE-BODY anchor, `^<!-- dispatcher-liveness-watchdog: ... -->[[:space:]]*$` (was `^<!-- dispatcher-liveness-watchdog: ... -->($|\n)` pre-round-6, which tolerated trailing report prose because the marker used to be embedded as the report's first line; now that the marker is always its own comment, the anchor requires the ENTIRE body to be the marker) — that fall STRICTLY AFTER the latest tier-2 trip-report cutoff, sorts by `createdAt`, takes the **last** qualifying match. **[codex review, PR #472, BLOCKING #2]** Without this cutoff, the tier-2 report's marker (posted per R4's "every evaluated tick" rule, including the trip tick, but — round 6 — as a comment strictly PRECEDING the report rather than embedded in it) would be read back by the very next evaluation as "the prior marker" — so an operator who fixes the park and removes `stalled` with the fingerprint otherwise unchanged would find the issue immediately re-tripped to `stalled` again instead of starting a fresh liveness episode. The producer's marker-before-report post ORDER (never the reverse) is what guarantees the marker's `createdAt` never exceeds the cutoff the report itself sets. **[codex review, PR #472, round 7 BLOCKING]** The cutoff itself is computed by matching `startswith($heading)` against `_LIVENESS_TIER2_HEADING` (`lib-liveness.sh`) — the tier-2 report's exact opening line, single-sourced so the TIER2REPORT heredoc (`lib-dispatch.sh`) and this detector can never drift apart — NOT the round-6-era unanchored `contains("Liveness watchdog tripped")` substring test it replaced. That unanchored test let ANY comment merely mentioning the bare phrase anywhere in its body (a human collaborator discussing a past trip in prose, with no report structure at all) register as a forged trip and move the cutoff past the genuine earlier marker, permanently resetting a still-frozen issue's series to `count=1` on every subsequent tick.
