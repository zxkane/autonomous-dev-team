...
| `issue_labeled` | dispatcher (Step 2 dev-new) | `labeled_at`? (ISO-8601, the real `autonomous`-label time, read via the `itp_label_event_ts` ITP verb — [INV-93], observe-only/non-blocking) | issue first picked up — TTHW "labeled" endpoint (first dispatch only). The event `ts` is the dispatch instant (can lag the label by ticks); the aggregator prefers `labeled_at` when present so queued wait is counted. The verb returns empty on any failure / leaf-absent, so `labeled_at` is simply omitted and the aggregator falls back to `ts` |
...
| `pr_diff_soft_cap` | review wrapper (once per enabled review round, BEFORE the fan-out — [INV-124], issue #452) | `side`, `pr`, `over_reach` (bool), `changed_files`?, `changed_lines`?, `files_cap`?, `lines_cap`? | the PR-diff-size (over-reach) soft signal. Emitted ONLY when at least one of `PR_DIFF_SOFT_CAP_FILES`/`PR_DIFF_SOFT_CAP_LINES` is configured (both unset ⇒ feature disabled ⇒ no event at all, zero behavior change). `over_reach` is the OR-across-dimensions strict-`>` comparison (`lib-review-diffcap.sh::review_diff_over_reach`); `changed_files`/`changed_lines` are the measured stats (omitted when the provider-seam read — `chp_pr_diffstat` — could not determine that dimension); `files_cap`/`lines_cap` are the configured thresholds (omitted when that key is unset). Exactly ONE event per review round regardless of fan-out size — distinct from the per-member `review_agent_run`/`agent_drop` events, which would duplicate the same PR-level value N times. **Observe-only, never a gate**: `over_reach` is never read by any verdict-aggregation code path (see [INV-124](invariants.md#inv-124-the-pr-diff-size-over-reach-signal-is-a-default-off-fail-open-soft-signal-never-read-by-verdict-aggregation)). |
