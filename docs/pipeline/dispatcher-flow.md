...
## Pre-step: opportunistic lane GC ([INV-117](invariants.md#inv-117-a-periodic-box-wide-gc-reclaims-dead-lane-process-residue-under-a-decision-table-that-fails-toward-leak-never-toward-false-kill), #380/#384)

Before `kill_stale_wrapper` runs, `dispatch-local.sh` opportunistically invokes `adt-gc.sh --quick || true` — Pass 1 (registry-driven) only, flock-guarded (`flock -w 3`, never `-n`, so a quick call queues briefly behind a concurrent full run instead of starving). The P8 implementation candidate makes this path reclaim eligible dead-lane registry residue by default on Linux, but production rollout remains blocked on issue #384's at-least-two-week soak gate. `$ADT_STATE_ROOT/adt-gc.conf` with `ADT_GC_ENFORCE=0` vetoes an environment opt-in and makes both this path and the periodic timer dry-run for a box-wide rollback; an explicit CLI mode remains highest precedence. If the environment omits `ADT_STATE_ROOT`, the shared `lib-state-root.sh` resolver used by `lib-dispatch.sh`, `lib-lane.sh`, and the remote SSM liveness command reads the timer installer's mode-600 host pointer at `$HOME/.local/state/adt-state-root`, so a custom scheduler root cannot diverge between local and remote lane-marker consumers. Strict Pass 1 accepts only exact `BACKEND=pgid` plus durable Linux v2 process identities; every backend/identity ambiguity skips toward leak. `|| true` — GC is best-effort; a missing/broken `adt-gc.sh` (stale skill tree) never blocks or delays a dispatch.
