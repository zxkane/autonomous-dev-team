...
A tick where this fingerprint is unchanged from the prior tick is a **no-op tick**; the running count persists via a `<!-- dispatcher-liveness-watchdog: issue=<N> fingerprint=<hash> count=<n> tier1=<0|1> -->` marker, posted/updated on every evaluated tick (mirrors INV-122's "computed on EVERY round" requirement), read back via `_liveness_prior_marker` (`lib-liveness.sh`) — a CUTOFF-then-scan (mirrors `_review_cap_prior_marker`) that excludes any marker at or before the latest tier-2 "Liveness watchdog tripped" report, so an operator resuming a stalled issue (removing `stalled` with the fingerprint otherwise unchanged) starts a fresh episode instead of immediately re-tripping off the trip report's own embedded marker.
