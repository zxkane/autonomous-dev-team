...
> reads normalized mergeability between two provider snapshots of the same
> open HEAD. Current `MERGEABLE` or `CONFLICTING` requeues `pending-review` so
> the ordinary preflight reaches fan-out or the canonical conflict/rebase
> route. A successful current `UNKNOWN` read retains the existing stall.
> Mergeability-read failure retains the residual notice but returns operational
> defer, so the tick omits `JUST_DISPATCHED` and INV-128 remains an effective
> bound; the failed read is not fabricated as fresh `UNKNOWN`.
...
**Part 2 ([INV-125]): closing the counting hole.** Every marker-present / budget-exhausted arm above is terminal once its evidence is current. The [INV-149] exception first refreshes historical `mergeable-unknown`: known current states re-enter review, a read failure defers under INV-128, and only a successful fresh `UNKNOWN` observation reaches `mark_stalled`. Other non-substantive causes and the shared dev-new budget still call `mark_stalled` directly. A park posts only an idempotent one-shot notice, which `count_retries` never counts (no dispatch → no countable comment), so a marker-present fall-through with usable evidence would freeze `count_retries` forever with `MAX_RETRIES` never able to trip.
