...
**Monotonicity guard (closes the same-PR self-ratification bypass)**: the discovered-vs-declared reconcile above only proves the tree matches WHATEVER baseline ships in the same change — so a PR that BOTH adds a raw-gh AND regenerates the baseline (`--generate-baseline`) would satisfy it (a #286 review reproduced `gh issue view 123` + regenerate → exit 0). The guard therefore ALSO reads the **trusted (merged) baseline** at `--trusted-ref` (default `origin/main`; override via the flag or `CUTOVER_TRUSTED_REF`) with `git show <ref>:<path>` and FAILs if the working-tree baseline GREW — a new `(file,content)` signature OR a higher count for an existing one — naming the grown site. The baseline may therefore only ever SHRINK; ratifying a NEW raw-gh site is rejected even when the in-PR reconcile is satisfied. When not in a git work tree, or the ref / trusted baseline is unresolvable (shallow / fork checkout, or the very first PR that introduces the baseline), the monotonicity check SKIPS gracefully — the credential-free merge gate re-runs it with `origin/main` present, and the in-PR reconcile still anchors the tree meanwhile. The detector greps with `-a` (force text) so a script carrying UTF-8 punctuation is never misclassified "binary" and silently skipped.
