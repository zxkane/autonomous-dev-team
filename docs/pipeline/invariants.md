...
**Bot-unfixable detection is scoped to the ACTIVE dev attempt (#274 review [P1] findings 1 ×2)**: `dev_report_bot_unfixable` only counts a 403 signature when it was reported BY the current dev attempt, not when an earlier review/human/dev comment merely *quotes* the signature. Two scopings: (a) a lower bound at the current session's `Dev Session ID: <session_id>` comment timestamp — the moment the active attempt started (falling back to the current-HEAD review window — comments after the most recent `Reviewed HEAD:` trailer for a *different* SHA — then to "no lower bound" only on the genuinely-first cycle, so an old 403 self-expires once a newer cycle's trailer/session lands); and (b) an **exclusion of review-agent comments** (`Review Session:` / `Review findings` / `Review Agent:` markers) so a reviewer that quotes `Resource not accessible by integration` while *describing* the bug is never counted as the dev attempt hitting it. The escalation branch is **additionally** gated on `current_head == last_reviewed_head`: a 403 only blocks when HEAD has NOT advanced. Both bounds are fail-open toward NOT-unfixable, so a same-HEAD substantive failure whose active attempt did not actually hit the 403 still gets its bounded retry rather than a spurious stall. Without this scoping, a single historical or quoted 403 would permanently route every later completed-session substantive failure on the issue to `mark_stalled`, even after the underlying obstacle was cleared.
