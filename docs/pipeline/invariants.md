...
2. **`kill_stale_wrapper` escalation gate** (`dispatch-local.sh`, both the legacy PID-file path and the pgrep-fallback orphan sweep): the gate that decides whether to fire the SIGKILL pass changes from a bare `kill -0 "$old_pid"` (leader-only) to `_pid_or_group_alive "$old_pid"`, which checks BOTH the individual pid and its negative-pid (whole-group) form. A TERM-trapping member whose leader already died now correctly triggers the KILL pass.
...
**Test**: `tests/unit/test-lane-gc-p3-kill-paths.sh` — group whose leader died on TERM but a member traps TERM is KILLed within grace (both via direct `_kill_group_escalate` and via the full `install_agent_sigterm_trap` path against a registry pgid); `kill_stale_wrapper`'s leader-or-group gate fires the KILL pass against a TERM-trapping member whose leader already exited (both the PID-file path and the pgrep-fallback path); grep-pins that TERM precedes KILL at every kill site and that no `pkill -f 'autonomous-'` widening exists anywhere. `tests/unit/test-kill-before-spawn.sh`, `tests/unit/test-dispatch-local-pgrep-type-scope.sh`, `tests/unit/test-pid-guard-pgid.sh`, `tests/unit/test-pid-alive-long-running.sh`, and `tests/unit/test-lib-lane.sh` stay green (extraction snippets updated to also capture the new `_pid_or_group_alive` sibling function; `lane_kill`'s external behavior — TERM→grace→KILL over every distinct recorded pgid — is unchanged by the internal refactor onto `_kill_group_escalate`).
