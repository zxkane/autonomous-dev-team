...
**Bot-unfixable detection is HEAD-window-scoped (#274 review [P1] finding 1)**: `dev_report_bot_unfixable` only counts a 403 signature in comments created *after* the most recent `Reviewed HEAD:` trailer for a SHA **different** from the current HEAD — i.e. within the current HEAD's review cycle — so an old 403 self-expires once a newer review cycle's trailer lands. The escalation branch is **additionally** gated on `current_head == last_reviewed_head`: a 403 only blocks when HEAD has NOT advanced. If the dev pushed new commits (HEAD moved) or a maintainer applied the metadata edit (a re-review advanced the trailer), the prior 403 is stale and the branch falls through to a fresh `dev-new` against the new HEAD. Without this scoping, a single historical 403 would permanently route every later completed-session substantive failure on the issue to `mark_stalled`, even after the underlying obstacle was cleared.
