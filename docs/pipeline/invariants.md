...
**Status**: **DISPATCH SKELETON SHIPS; ITP READ leaves (#281) + ITP WRITE leaves (#283) code-enforced; dep + CHP refactor not yet.** The normative `provider-spec.md` landed with issue #279. The dispatch skeleton — `lib-issue-provider.sh` / `lib-code-host.sh` (the 13 ITP + 12 CHP thin shims routing `itp_<verb>` → `itp_${ISSUE_PROVIDER}_<verb>` / `chp_<verb>` → `chp_${CODE_HOST}_<verb>`) + the empty `providers/itp-github.{sh,caps}` / `chp-github.{sh,caps}` scaffolds + the `.caps` reader — ships in issue #280 with **ZERO verb leaves migrated and zero behavior change**. **The ITP READ leaves — `itp_github_list_by_state`/`itp_github_count_by_state`/`itp_github_list_forbidden_combos`/`itp_github_read_task`/`itp_github_list_comments` — are migrated in #281**: the six state-list callers route through the verbs with byte-identical `gh` argv, the 28 `gh issue view --json comments` marker-scanners consolidate behind `itp_list_comments` (returning the [INV-90] normalized array; the `capture`/`sort_by`/exact-eq parse stays caller-side), and the `title`/`body`/`state` reads route through `itp_read_task`. **The ITP WRITE leaves — `itp_github_transition_state`/`itp_github_post_comment`/`itp_github_edit_comment`/`itp_github_mark_checkbox`/`itp_github_provision_states` — are migrated in #283**: `label_swap` → `itp_transition_state`; all 18 `gh issue comment` sites (incl. the dispatcher markers `post_dispatch_token` [INV-18] / `_dep_block_comment` [INV-39]) → `itp_post_comment` ([INV-89]); the INV-46 SHA stamp (`lib-review-e2e.sh`) → `itp_edit_comment` (+ `edit_comment=0` fresh-marker fallback); `mark-issue-checkbox.sh` PATCH → `itp_mark_checkbox`; `setup-labels.sh` → `itp_provision_states` — all proven zero-behavior-change by golden-trace, with the marker text / retry / dedup / [INV-25] terminal-state jq subtraction staying caller-side. The remaining `gh`-site refactor (the ITP dep leaves and all CHP verbs) is later, separately-funded sibling issues (itp-deps-begin-tick / chp-pr-lifecycle / entangled-orchestrators-golden-trace). No wrapper behavior changes in #279, #280, #281, or #283.

**Producer**: `lib-issue-provider.sh` / `lib-code-host.sh` dispatchers + `providers/itp-github.sh` / `providers/chp-github.sh` (the shim layer + scaffolds exist as of #280; the ITP READ `itp_github_<verb>` bodies are populated by #281, the ITP WRITE bodies by #283; the dep + CHP leaf bodies are populated by the downstream migration issues). **Consumer**: every caller in `lib-dispatch.sh` / the wrappers that today emits a `gh` call.

**Tests**: `tests/unit/test-provider-spec.sh` asserts the spec lists all 25 verbs and names the dispatcher's own marker writers. `tests/unit/test-provider-dispatch.sh` (#280, updated #281/#283) asserts the dispatch layer routes `itp_<verb>` → `itp_github_<verb>` / `chp_<verb>` → `chp_github_<verb>` under the default `github` provider, that all 13 ITP + 12 CHP shims are defined, that the libs resolve `providers/` via `readlink -f` of `BASH_SOURCE`, that the migrated ITP READ+WRITE leaf bodies are now DEFINED, and that the dep + CHP verbs are still empty scaffolds. `tests/unit/test-itp-read-leaves.sh` (#281) and `tests/unit/test-itp-write-leaves.sh` (#283) are the golden-trace / dispatch-routing / `.caps`-parse / capability-branch suites for the ITP READ and WRITE leaves respectively. The golden-trace runtime tests for the dep + CHP leaves gate their respective code-bearing siblings (per [`provider-spec.md`](provider-spec.md) §7).
...
**Status**: **IMPLEMENTED for GitHub in #283** (spec-defined with #279). The ITP WRITE leaf `itp_github_post_comment` (`providers/itp-github.sh`) is the single byte-identical `gh issue comment "$issue" --repo "$REPO" --body "$BODY"` choke-point, and ALL 18 `gh issue comment` sites in `lib-dispatch.sh` — including the dispatcher's own `post_dispatch_token` ([INV-18]) and `_dep_block_comment` ([INV-39]) markers — now route through `itp_post_comment` with the marker BODY composed caller-side (`grep -c 'gh issue comment' lib-dispatch.sh` == 0). GitHub's `marker_channel=html` is today's behavior (zero change); the `text`-channel branch is the Asana path that ships with the Asana provider. The read-side `capture()` scanners already consolidated behind `itp_list_comments` ([INV-90], #281).

**Producer**: `itp_github_post_comment` (the write leaf, #283) + the per-provider `marker_channel` cap. **Consumer**: every marker writer (agent + `post_dispatch_token` + `_dep_block_comment`, now all via `itp_post_comment`) and every `capture()`-style read-side scanner.

**Tests**: `tests/unit/test-provider-spec.sh` asserts the spec pins `marker_channel=html` for GitHub and documents the agent-AND-dispatcher coverage + the read-side channel branch. `tests/unit/test-itp-write-leaves.sh` (#283) golden-traces `itp_github_post_comment` byte-identical (incl. the INV-18 `dispatcher-token` + INV-39 `dep-block` marker bodies verbatim), pins the html channel does NOT strip `<!-- … -->`, asserts `grep -c 'gh issue comment' lib-dispatch.sh` == 0, and exercises the `marker_channel=text` / `edit_comment=0` degraded-provider branches.
