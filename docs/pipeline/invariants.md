...
1. **Same launch path the fan-out uses.** Each member's smoke resolves its model via `_resolve_review_agent_model` ([INV-41](#inv-41-per-agent-review-model--extra-args-resolution)) and applies the [INV-38](#inv-38-per-side-agent_launcher-precedence) / [INV-42](#inv-42-per-agent-review-launcher-resolution) launcher treatment (a per-agent `AGENT_REVIEW_LAUNCHER_<AGENT>` opt-in else the INV-38 rule: non-claude → neutralized, claude → keeps the rebound `AGENT_LAUNCHER_ARGV`) — IDENTICAL to the fan-out's per-agent subshell — so the smoke exercises the exact launch chain the real review run would. A smoke PASS therefore certifies the same `(CLI, model, launcher)` tuple the fan-out is about to run.
