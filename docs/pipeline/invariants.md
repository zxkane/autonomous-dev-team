...
Empty/unset model → no `--model` (agy uses its config default). The enumeration is cached once per process in `_LIB_AGENT_AGY_MODELS_CACHE`; the match is **fixed-string, whole-line** (`grep -Fxq`) so a prefix or a regex-metachar string never matches. The model is **control-char-stripped** (`${model//[[:cntrl:]]/}` — a newline *and* a carriage return) before both the `grep -Fxq` check (in `_agy_known_model`) and the `--model` forward (up-front in `_agy_build_model_args`, so the SAME sanitized value is validated and forwarded) — a multi-line value would otherwise split across cache lines and bypass the whole-line match, and a trailing `\r` would whole-line-match a CRLF listing yet survive into the argv. Both sites strip the SAME `[[:cntrl:]]` class, mirroring the [INV-60](#inv-60-the-review-model-is-shown-inline-on-every-verdict-comments-review-agent-line) `[[:cntrl:]]` guard in `post-verdict.sh` so the two model sites agree. **Do not remove either strip, nor narrow the class back to `\n`-only**; both are load-bearing security hardening (PR #192). The enum-failure sentinel is `\x01`-wrapped specifically so no real `agy models` line can collide with it.
...
**Producer**: `_agy_known_model` + `_agy_build_model_args` in `skills/autonomous-dispatcher/scripts/adapters/agy.sh` ([INV-75](#inv-75-all-per-cli-behavior-lives-in-that-clis-adapter--inline-cli-conditionals-in-orchestration-code-are-a-defect), #232), called from `adapter_invoke_agy` (`run_agent` / `resume_agent` in `lib-agent.sh` dispatch to it).
...
**Test**: `tests/unit/test-lib-agent-agy.sh` — AGY-06a (known model → single-argv `--model`), AGY-06b (empty → no `--model`, no WARN), AGY-06b2 (unknown → omitted + WARN naming value + `AGENT_REVIEW_MODEL_AGY`, rc 0), AGY-06b3 (`agy models` failure → best-effort pass-through), TC-AGYM-KM (`_agy_known_model`: known/unknown/prefix/regex-metachar/empty + **embedded-newline injection rejected** + once-per-process caching), AGY-06e / TC-AGYM-BM (known model carrying a trailing newline → the FORWARDED `--model` argv is the sanitized newline-free value, asserted via NUL-delimited argv recorder — PR #192), AGY-06e2 / TC-AGYM-BM2 (same for a trailing **carriage return** — the `[[:cntrl:]]` strip covers `\r` too, so a `\r`-bearing known id validates clean yet forwards sanitized — PR #192, F2), AGY-06c (resume `--conversation` path forwards `--model`), AGY-06d (resume-fallback threads `--model` through `run_agent`), AGY-WARN-GONE (old `does not support --model` string removed from source). Test plan: `docs/test-cases/agy-model-support.md`.
...
- #296 B5+B7 (#310): live-wrapper issue READS — autonomous-dev.sh issue-body (1× itp_read_task), autonomous-review.sh no-auto-close label gate (1× itp_read_task) — byte-identical; baseline shrank by 2 sigs.
