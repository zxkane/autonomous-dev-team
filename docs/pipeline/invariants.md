...
**Status**: **DISPATCH SKELETON SHIPS; `gh`-site refactor not yet code-enforced.** The normative `provider-spec.md` landed with issue #279. The dispatch skeleton — `lib-issue-provider.sh` / `lib-code-host.sh` (the 13 ITP + 12 CHP thin shims routing `itp_<verb>` → `itp_${ISSUE_PROVIDER}_<verb>` / `chp_<verb>` → `chp_${CODE_HOST}_<verb>`) + the empty `providers/itp-github.{sh,caps}` / `chp-github.{sh,caps}` scaffolds + the `.caps` reader — ships in issue #280 (the dispatch-skeleton issue) with **ZERO verb leaves migrated and zero behavior change**. The actual `gh`-site refactor (moving each leaf behind a verb) and the caller rewire are later, separately-funded sibling issues (itp-reads / itp-writes / chp-pr-lifecycle / entangled-orchestrators-golden-trace). No wrapper behavior changes in #279 or #280.

**Producer**: `lib-issue-provider.sh` / `lib-code-host.sh` dispatchers + `providers/itp-github.sh` / `providers/chp-github.sh` (the shim layer + empty scaffolds exist as of #280; the `itp_github_<verb>` / `chp_github_<verb>` leaf bodies are populated by the downstream migration issues). **Consumer**: every caller in `lib-dispatch.sh` / the wrappers that today emits a `gh` call.

**Tests**: `tests/unit/test-provider-spec.sh` asserts the spec lists all 25 verbs and names the dispatcher's own marker writers. `tests/unit/test-provider-dispatch.sh` (#280) asserts the dispatch layer routes `itp_<verb>` → `itp_github_<verb>` / `chp_<verb>` → `chp_github_<verb>` under the default `github` provider, that all 13 ITP + 12 CHP shims are defined, and that the libs resolve `providers/` via `readlink -f` of `BASH_SOURCE`. The golden-trace runtime tests gate the code-bearing leaf-migration siblings (per [`provider-spec.md`](provider-spec.md) §7), NOT #280 — no leaf carries a `gh` argv yet.
...
**Status**: **CAPS MANIFESTS + READER SHIP** with the dispatch skeleton (#280). The `.caps` manifests (`providers/itp-github.caps`, `providers/chp-github.caps`) declaring exactly today's GitHub behavior, and the parsed-not-sourced `itp_caps`/`chp_caps` reader, land in #280. The 9 ITP + 4 CHP capability keys and their github/gitlab/asana columns are normative in [`provider-spec.md`](provider-spec.md) §4.1/§4.2. The capability *branches* in callers ship with the downstream leaf-migration issues; #280 proves each `caps=0` branch is reachable via a named degraded fake fixture provider, with zero behavior change.
