...
**Tests**: `tests/unit/test-provider-spec.sh` asserts the spec pins `marker_channel=html` for GitHub and documents the agent-AND-dispatcher coverage + the read-side channel branch. `tests/unit/test-itp-write-leaves.sh` (#283) golden-traces `itp_github_post_comment` byte-identical (incl. the INV-18 `dispatcher-token` + INV-39 `dep-block` marker bodies verbatim), pins the html channel does NOT strip `<!-- … -->`, asserts ZERO raw `gh issue comment` across all five ITP-issue-comment files (`lib-dispatch.sh`, `autonomous-{dev,review}.sh`, `dispatcher-tick.sh`, `lib-review-verdict.sh`), and exercises the `marker_channel=text` / `edit_comment=0` degraded-provider branches. `tests/unit/test-autonomous-review-verdict-trailer.sh` confirms `emit_verdict_trailer` still emits the byte-identical trailer through the verb.
