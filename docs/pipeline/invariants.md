...
## INV-116: a periodic, box-wide GC reclaims dead-lane process residue under a decision table that fails toward leak, never toward false-kill — dry-run by default
...
**Numbering note**: the design doc (`docs/designs/lane-containment-gc.md` §8) drafted this as INV-110 at design time. By this PR's open, PR-2 (#378) had already shipped INV-109/INV-110 and PR-3 (#379/#405) had shipped INV-114/INV-115 (each already renumbered once from the design's own draft — see `docs/designs/lane-gc-p3-kill-paths.md`). Current head at this PR's open is INV-115; per the repo-wide renumber-on-collision convention, this PR claims the first free slot, **INV-116**.
