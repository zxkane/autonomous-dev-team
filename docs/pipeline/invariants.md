...
2. **Liveness deferral**: `mark_stalled` MUST query `pid_alive --at-cap issue <issue_num>` before any `gh issue edit` or stall comment. If the dev wrapper is alive, the stall decision is deferred — `mark_stalled` returns 0 without changing labels, posts a one-shot deferral comment (idempotency-keyed on the wrapper PID via marker `INV-26-stall-deferral:pid=<pid>`), and lets the next tick re-evaluate. The stall path proceeds only when no live wrapper holds the PID file.

   **At-cap call (issue #263)**: `mark_stalled` is the *only* `pid_alive --at-cap` caller (the flag is a positional argument, never an env var, so it cannot leak to other callers). It is reached from exactly one site (`dispatcher-tick.sh` Step 4) and only when `count_retries >= MAX_RETRIES` — the at-cap state. Under `EXECUTION_BACKEND=remote-aws-ssm`, the `--at-cap` flag makes a persistently *indeterminate* SSM liveness verdict return DEAD instead of biasing ALIVE (see [INV-30]'s at-cap exception). This bounds the deferral to ONE tick once retries are exhausted, fixing the unbounded-defer hang where a flaky transport kept `mark_stalled` deferring forever and the `stalled` label was never written (a downstream consumer's ~40h hang). Definite ALIVE/DEAD verdicts are unaffected.

   **Empty-PID = DEAD shortcut, narrowed to local backend (issue #263)**: when the backend is `local` (or unset = local default) AND `get_pid` returns empty AND `pid_alive` returned ALIVE (only reachable under local backend via the tier-2 PID-file-mtime or tier-3 heartbeat-mtime fallbacks, where the PID file *content* can be empty while its mtime is fresh), `mark_stalled` treats the empty PID as DEAD: it skips the one-shot deferral comment entirely and proceeds to write `stalled` — an empty PID means no wrapper is running. This shortcut MUST NOT apply under `remote-aws-ssm`: there the PID file lives on the wrapper box and dispatcher-side `get_pid` is *always* empty regardless of wrapper state, so empty-PID is the steady state, not a DEAD signal — the `--at-cap` flag handles the indeterminate verdict instead. Applying the shortcut under the remote backend would resurrect the #121 / downstream-consumer false-stall bug.
...
- (2) `lib-dispatch.sh::mark_stalled` — calls `pid_alive --at-cap issue $issue_num` first; defers (post one-shot comment, return 0) if alive, except under local backend with an empty PID (treated as DEAD, no deferral comment).
...
**At-cap exception (issue #263)**: `pid_alive` accepts an optional leading positional `--at-cap` flag. When set, an *indeterminate* remote verdict returns 1 (DEAD) instead of biasing ALIVE. This does NOT weaken the conservative-bias rationale above — it is a deliberate **policy choice** scoped to the one caller that has already proven the wrapper's retry budget is exhausted (`mark_stalled`, [INV-26]). At-cap, the trade-off in the table flips: continuing to bias ALIVE on a flaky transport produces the *unbounded* defer loop (the `stalled` label is never written; a downstream consumer hung ~40h), whereas returning DEAD costs at most a false stall on a genuinely-alive wrapper whose recovery is the same one-line `gh issue edit --remove-label stalled` as any other false stall. Framed precisely: **at-cap indeterminate means "stop waiting", NOT "proved dead".** Scope guards:
...
- [INV-26] (stall decision deferral on live wrappers) — `mark_stalled` calls `pid_alive --at-cap` first; under remote backend that call inherits the remote query automatically, and the `--at-cap` flag (issue #263) caps the indeterminate-defer loop at one tick so a persistently-flaky transport can't strand a retry-exhausted issue in `pending-dev` forever. `mark_stalled` is the sole `--at-cap` caller.
