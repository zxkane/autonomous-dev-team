...
`release_dispatch_marker <issue_num> <mode>` (the shared removal primitive both the explicit soft-fail call sites and the EXIT-trap handler use) is idempotent and best-effort — `rm -rf` on an absent path is a silent no-op, and a removal failure (e.g. permissions) is swallowed rather than raised, so a marker that fails to release simply falls back to the pre-existing TTL-expiry safety net rather than becoming a new hard-error surface.
