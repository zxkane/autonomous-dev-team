...
2. **Pass 2 — tagged-orphan sweep** (same-uid process enumeration; `ps -eo pid= -U "$(id -u)" | grep -vw "$$"` — the self-match-safety line the design's own attack ledger calls out). A candidate is kill-eligible **only when ALL** of: (a) its env carries `ADT_LANE_ID=<X>` where lane X resolves DEAD via Pass 1's own rules with a 300s age floor, **or** the legacy signature `AUTONOMOUS_CONF_LOADED_FROM` present **AND** `CC_USER=autonomous-{dev,review}-bot` **AND** `ppid==1`, with a 600s floor (the `∧`, never `∨` — a bare conf-sourcing in an operator's own SSM debug shell must never authorize a kill); an `ADT_LANE_ID` absent from every registry is an unconditional skip (`unknown_class`), never treated as legacy; (b) no `TERM_PROGRAM` in its env (operator tooling is unconditionally untouchable); (c) its pgid contains no live-wrapper argv match (`pgrep -g $PG -f 'autonomous-(dev|review)\.sh'`, survives comm truncation and a launcher-bridge exec chain); (d) its pgid is outside every live lane's recorded `pgids`, outside every live PID-file's pgid, and outside the descendant ancestry of every live wrapper process **box-wide** (not scoped to one `PROJECT_DIR` — see the design-note interpretation below); (e) process age clears the applicable floor. Escalation is TERM → 10s grace → KILL (group-form via the shared `_kill_group_escalate`, individual-pid fallback). Banned as kill keys, full stop: `CLAUDE_CODE_SESSION_ID`, kernel sid liveness, a bare `ppid==1` on its own, and any comm/process-name match.
3. **Pass 3 — env-blind classes**: a Chrome process whose `--user-data-dir` argv matches a dead lane's recorded `CHROME_PROFILE_HINT` (3.1); a heuristic Chrome match (`/tmp/puppeteer_dev_chrome_profile-*`, `ppid==1`, age > 2h, no `TERM_PROGRAM`, no live-wrapper-ancestry) (3.2); a wedged `gh pr checks --watch`/`gh api` whose `GH_TOKEN_FILE` directory no longer exists on disk (3.3); an E2E server whose recorded lane `WORKTREE` no longer exists and whose `cwd` was under it (3.4). Crashpad/helper processes are never judged by name — only by falling through to Pass 2's env-based join, per the design's explicit "judged by intact env via Pass 2, never by ppid/name" rule (3.5).
...
**New `lib-lane.sh` portability primitives** (design §12 R13: helpers live in the lib, `adt-gc.sh` stays a thin entry-point): `proc_argv` (Linux `/proc/PID/cmdline`, macOS via the procargs2 shim), `proc_ppid` (Linux `/proc/PID/stat` field 4, macOS/BSD `ps -o ppid=`), `proc_pgid` (`ps -o pgid=`, identical both platforms), and `env_lookup` (an exact-key convenience wrapper over the existing `env_of`). None of these change any existing function's behavior.
