...
**Status**: **DISPATCH SKELETON SHIPS; ITP READ leaves code-enforced (#281); WRITE/dep + CHP refactor not yet.** The normative `provider-spec.md` landed with issue #279. The dispatch skeleton — `lib-issue-provider.sh` / `lib-code-host.sh` (the 13 ITP + 12 CHP thin shims routing `itp_<verb>` → `itp_${ISSUE_PROVIDER}_<verb>` / `chp_<verb>` → `chp_${CODE_HOST}_<verb>`) + the empty `providers/itp-github.{sh,caps}` / `chp-github.{sh,caps}` scaffolds + the `.caps` reader — ships in issue #280 with **ZERO verb leaves migrated and zero behavior change**. **The ITP READ leaves — `itp_github_list_by_state`/`itp_github_count_by_state`/`itp_github_list_forbidden_combos`/`itp_github_read_task`/`itp_github_list_comments` — are migrated in #281**: the six state-list callers (`count_active`/`list_new_issues`/`list_pending_review`/`list_pending_dev`/`list_stale_candidates`/`list_hygiene_residue`) route through the verbs with byte-identical `gh` argv, the 28 `gh issue view --json comments` marker-scanners consolidate behind `itp_list_comments` (returning the [INV-90] normalized array; the `capture`/`sort_by`/exact-eq parse stays caller-side), and the `title`/`body`/`state` reads at `autonomous-dev.sh:1097` / `status.sh:85` route through `itp_read_task` — all proven zero-behavior-change by golden-trace. The remaining `gh`-site refactor (ITP WRITE leaves, the dep leaves, and all CHP verbs) is later, separately-funded sibling issues (itp-writes / itp-deps-begin-tick / chp-pr-lifecycle / entangled-orchestrators-golden-trace). No wrapper behavior changes in #279, #280, or #281.

**Producer**: `lib-issue-provider.sh` / `lib-code-host.sh` dispatchers + `providers/itp-github.sh` / `providers/chp-github.sh` (the shim layer + scaffolds exist as of #280; the ITP READ `itp_github_<verb>` bodies are populated by #281; the WRITE/dep + CHP leaf bodies are populated by the downstream migration issues). **Consumer**: every caller in `lib-dispatch.sh` / the wrappers that today emits a `gh` call.

**Tests**: `tests/unit/test-provider-spec.sh` asserts the spec lists all 25 verbs and names the dispatcher's own marker writers. `tests/unit/test-provider-dispatch.sh` (#280, updated #281) asserts the dispatch layer routes `itp_<verb>` → `itp_github_<verb>` / `chp_<verb>` → `chp_github_<verb>` under the default `github` provider, that all 13 ITP + 12 CHP shims are defined, that the libs resolve `providers/` via `readlink -f` of `BASH_SOURCE`, that the migrated ITP READ leaf bodies are now DEFINED, and that the WRITE/dep + CHP verbs are still empty scaffolds. `tests/unit/test-itp-read-leaves.sh` (#281) is the golden-trace / dispatch-routing / `.caps`-parse / normalized-shape / capability-branch suite for the ITP READ leaves. The golden-trace runtime tests for the WRITE/dep + CHP leaves gate their respective code-bearing siblings (per [`provider-spec.md`](provider-spec.md) §7).
