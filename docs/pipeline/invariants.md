...
**Bot-unfixable detection is scoped to the current HEAD's review cycle (#274 review [P1], findings refined across rounds)**: `dev_report_bot_unfixable` only counts a 403 signature reported within the current HEAD's cycle, not one an earlier-cycle or review/human comment merely *quotes*. Two scopings: (a) a lower bound at the most recent `Reviewed HEAD:` trailer for a SHA *different* from `current_head` — i.e. when the current HEAD became the reviewed HEAD — falling back to "no lower bound" only on the genuinely-first cycle, so an old 403 self-expires once a newer cycle's trailer lands; and (b) an **exclusion of review-agent comments** (`Review Session:` / `Review findings` / `Review Agent:` markers) so a reviewer that quotes `Resource not accessible by integration` while *describing* the bug is never counted as the dev attempt hitting it. The bound is deliberately NOT anchored at the dev session's `Dev Session ID:` comment: that comment is the `Agent Session Report (Dev)` trailer the wrapper posts in `cleanup()` at the END of the run, *after* the agent has already posted its completion/blocking note explaining the 403 — anchoring there would put the real 403 BEFORE the bound and miss it (#274 review [P1] round-4 finding 2). The escalation branch is **additionally** gated on `current_head == last_reviewed_head`: a 403 only blocks when HEAD has NOT advanced. Both bounds are fail-open toward NOT-unfixable, so a same-HEAD substantive failure whose attempt did not actually hit the 403 still gets its bounded retry rather than a spurious stall. Without this scoping, a single historical or quoted 403 would permanently route every later completed-session substantive failure on the issue to `mark_stalled`, even after the underlying obstacle was cleared.
