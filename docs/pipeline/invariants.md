...
1. **Failure counter accuracy**: `count_agent_failures` (`lib-dispatch.sh`) MUST exclude Session Reports whose `Exit code` is 0 (success), 143 (SIGTERM), or 137 (SIGKILL) from the agent-failure tally. Exits 143 and 137 are almost exclusively caused by `dispatch-local.sh::kill_stale_wrapper` — the dispatcher's own kill of a stale wrapper to spawn a fresh one. Counting the dispatcher's kill as an "agent failure" silently consumes retry budget the agent never spent. **[Lane-GC PR-3 / INV-111 attribution]**: this exclusion covers rc=137 regardless of WHICH kill path produced it — a SIGKILL delivered by `_kill_group_escalate`'s bounded grace-then-KILL escalation (the wrapper's own TERM trap, `kill_stale_wrapper`'s leader-or-group gate, or `lane_kill`) is exactly as dispatcher-induced as a bare `kill -9`; every one of those paths is TERM-first per [INV-111], so an rc=137 that follows a pipeline-initiated TERM is self-induced, not a crash, and must not be counted against retry budget.
...
- [INV-111] (kill-escalation contract) — every pipeline-initiated kill path (wrapper TERM trap, `kill_stale_wrapper`, `lane_kill`) is TERM-first with a bounded grace before any SIGKILL, which is what makes rule (1)'s rc=137 exclusion correct: the escalation NEVER SIGKILLs a process the pipeline hasn't already TERMed.

**Status**: **ENFORCED** (closes #121 Fix A + Fix C; Fix B / `dev_near_success` is a separate follow-up PR; the at-cap ceiling + empty-PID local-backend shortcut added in #263; the [Lane-GC PR-3 / INV-111] attribution note added for issue #379).
...
3. **A per-run timeout OR any other signal-death STOPS the loop immediately and returns the INV-48 veto rc (zero further re-runs).** A `124` (coreutils `timeout`'s OWN TERM-expiry exit code — NOT 128+signal, `timeout` itself exits 124) or **any** `128+N` signal-death rc — `137` (`--kill-after` SIGKILL) and, as of [INV-112](#inv-112-a-codex-review-signal-death-rc-including-143-terminates-the-re-run-loop-and-a-defense-in-depth-reap-terminates-the-fan-out-controller-subshell-itself-406) (#406), **`143` (SIGTERM) and every other `128+N`** — terminates the re-run loop at once and `_run_codex_review` returns that rc, so the post-window sweep maps a no-verdict 124/137 to `timed-out` (a deciding FAIL that VETOES the merge per [INV-48](#inv-48-per-side-review-wall-clock-timeout-agent_review_timeout-1h-default-with-browser-e2e-exclusion-and-timeout-veto)). The re-run loop exists for TRANSIENT stream errors, **not** a process-level kill: re-running a wall-clock-capped OR externally-terminated run is pointless (the cap refires, or the terminator — the wrapper's own reap, the operator, or system shutdown — will terminate the fresh run too) and — because each clean re-run is a fresh `codex review` that may self-post — would risk DUPLICATE verdict comments (violating sub-rule 4's "exactly one verdict") and would also pin `AGENT_LAUNCH_RC` at the signal-death rc, which the sub-rule 5 rc-0 gate reads as "not a completed review" and refuses the stdout fallback for. An earlier draft keyed the loop break on the STICKY return value so a turn-1 timeout followed by a clean re-run kept the return rc at 124, never broke, and looped to `CODEX_REVIEW_MAX_RERUNS` — exactly that duplicate-verdict / refused-fallback hazard (#218 review finding 4). Stopping on the FIRST terminal rc (turn 1 or mid-loop, after some stream-error re-runs) closes it: a non-terminal exhaustion still returns the last rc → the poller resolves `unavailable`. See [INV-112](#inv-112-a-codex-review-signal-death-rc-including-143-terminates-the-re-run-loop-and-a-defense-in-depth-reap-terminates-the-fan-out-controller-subshell-itself-406) for why `143` specifically needed this generalization (the wrapper's own post-resolution reap SIGTERMs a still-running `codex review`, and pre-#406 that 143 was mis-classified as a #209 transient blip).
...
- [INV-112](#inv-112-a-codex-review-signal-death-rc-including-143-terminates-the-re-run-loop-and-a-defense-in-depth-reap-terminates-the-fan-out-controller-subshell-itself-406) — the #406 amendment to sub-rule 3's terminal-rc set, plus the defense-in-depth controller-subshell reap and fan-out-dir liveness gates.
- [`review-agent-flow.md` § codex review subcommand (INV-62)](review-agent-flow.md#codex-review-subcommand-inv-62) — runtime walkthrough.
...
1. **`adapters/codex.sh::_run_codex_review`'s re-run loop treats a signal-death rc as loop-terminal, not a [INV-62](#inv-62-the-codex-review-lane-runs-the-codex-review-subcommand-auto-scoped-prompt-carried-gate-with-a-stdout-verdict-fallback) sub-rule-2 #209 transient blip.** The loop breaks on `last_run_rc -eq 124` (coreutils `timeout`'s own TERM-expiry exit code) **OR** `last_run_rc -ge 128` (any `128+N` signal-death rc — 137 SIGKILL, 143 SIGTERM, and every other signal, generalized rather than enumerated). The wrapper's own INV-43/INV-84 post-resolution reap (`_reap_fanout_processes` / `_reap_fanout_recorded_descendants`) delivers SIGTERM to a still-running `codex review` whose verdict already resolved via a sibling agent or an early artifact land; pre-#406 that SIGTERM's rc 143 fell through to the #209 arm and scheduled a FRESH `codex review` — the token-burning orphan this issue reports. This layer ALONE ends the orphan for the observed incidents: the reap's SIGTERM makes `_run_with_timeout` return 143 → the loop breaks → the fan-out subshell running the controller runs off its own end and exits. The rc-2 clap-rejection gate ([INV-62](#inv-62-the-codex-review-lane-runs-the-codex-review-subcommand-auto-scoped-prompt-carried-gate-with-a-stdout-verdict-fallback) sub-rule 2, #223) and the INV-73 malformed-rc0 re-run path are UNCHANGED — both key on rc values `< 128`, so they never reach this gate.

2. **Defense-in-depth: `lib-review-poll.sh::_reap_fanout_controller_subshells` reaps the fan-out CONTROLLER SUBSHELL itself, at the same post-resolution reap call site as the existing two reapers, and the recorded-descendant marker sweep RUNS AGAIN immediately after.** Neither existing reaper reaches the controller: `_reap_fanout_processes` group-kills the AGENT's setsid PGID (`codex review`'s own process group, one level BELOW the controller — killing it is what triggers layer 1's break, but does not itself touch the shell loop that decides whether to launch AGAIN); `_reap_fanout_recorded_descendants` matches `ADT_FANOUT_LANE_MARKER` via `/proc/<pid>/environ`, but the marker is `export`ed INSIDE the already-running controller subshell, which does not rewrite that process's OWN already-execve'd environ (verified empirically — a shell's `export` mutates its in-memory environment table, but nothing re-writes `/proc/<pid>/environ` for the CURRENTLY RUNNING process itself). So `autonomous-review.sh` additionally calls `_reap_fanout_controller_subshells "${_fanout_pids[@]:-}"` — the SAME array the fan-out loop already collects each subshell's `$!` into (INV-40 sub-rule 1). This reaper uses ONLY a **direct, non-group** `kill "$pid"` / `kill -9 "$pid"`: the controller subshell is a plain fork with no `setsid`/`set -m` of its own, so it shares the WRAPPER's own process group, and a group-form `kill -- -$pid` would target the wrapper itself. These PIDs are **never** `lane_record_pgid`'d into the durable lane registry — doing so would let a later `lane_kill` (the Lane-GC delegate, [INV-109](#inv-109-every-wrapper-run-mints-and-atomically-installs-a-durable-lane-registry-entry-before-spawning-any-background-child-including-token-daemons-and-heartbeat)) kill the LIVE wrapper (the subshell's pgid IS the wrapper's pgid). **The late-spawn race (review-round-2 finding)**: a controller that already passed its layer-3 liveness-dir check and committed to launching another `codex review` before the fan-out dir was removed can still fork that ONE marked child in the narrow window BETWEEN the FIRST marker sweep (which ran before this controller kill and could not yet see a not-yet-spawned child) and the controller-PID kill itself — that child inherits `ADT_FANOUT_LANE_MARKER` from the subshell's exported env, but the first sweep already ran, and its own PGID is never sidecar-recorded (the fan-out dir is already gone by this point, so no `.pgid` sidecar is ever written for it). `autonomous-review.sh` therefore calls `_reap_fanout_recorded_descendants "ADT_FANOUT_LANE_MARKER" "${AGENT_SESSION_IDS[@]:-}"` a SECOND time, immediately after `_reap_fanout_controller_subshells` — idempotent against every PID the first pass already reaped (already dead → silent skip) and reaching exactly the late-spawned child the first pass structurally could not see yet.

3. **Fan-out-dir liveness gates prevent a spurious re-launch and silence the resulting rc-sidecar write error, on the path where layer 1's SIGTERM-driven break has not yet fired.** `_run_codex_review` takes an optional 5th arg — the wrapper's per-run fan-out scratch dir (`_FANOUT_DIR`, the SAME dir holding the agent's `.rc`/`.pgid` sidecars). Immediately before EVERY re-run launch (checked fresh each iteration, not just once — the dir can disappear WHILE the loop is mid-re-run, since INV-84 early resolution races the re-run controller), the loop checks the dir still exists; if the wrapper already `rm -rf`'d it (verdict resolution + sidecar collection already happened), the loop logs and breaks instead of spawning. Omitted/empty (every pre-#406 call site, including every unit test) is a byte-for-byte no-op — `_run_codex_review` must stay callable standalone without wrapper-internal plumbing. Symmetrically, the wrapper's own post-loop rc-sidecar write (`printf '%s\n' "$_rc" 2>/dev/null > "$_agent_rc_file"`) is guarded on `[[ -d "$_FANOUT_DIR" ]]`, so a deleted dir produces silence rather than a loud `No such file or directory` stderr line for a value nobody will ever read again. **The redirect order is load-bearing**: bash opens redirects left-to-right, so `> "$_agent_rc_file" 2>/dev/null` still attempts the `>` open FIRST and lets that open's ENOENT hit stderr before the dup2-to-`/dev/null` ever applies — it does NOT suppress the error. Only `2>/dev/null > "$_agent_rc_file"` (stderr redirected first) actually silences the narrow TOCTOU race where the dir vanishes between the `[[ -d ]]` check and this write's own `>` open.

**Missing-rc tolerance (pre-existing contract, load-bearing here)**: a controller subshell reaped by layer 2 before it wrote its `.rc` sidecar is handled by `autonomous-review.sh`'s pre-existing sidecar-missing branch (`AGENT_LAUNCH_RC["$_sid"]=1` when `[[ -f "$_rc_file" ]]` is false) — no crash, no hang; verdict resolution for every OTHER agent is unaffected because it is artifact-sourced (INV-78), not rc-sourced.

**Why**: the wrapper's INV-43/INV-84 reap was designed to bound a review round's process lifetime, but two structural gaps let a codex re-run controller outlive it: (1) rc 143 (the reap's OWN signal) was misclassified as a #209 transient stream blip rather than a terminal signal-death, so the loop scheduled a fresh `codex review` instead of stopping; (2) even with (1) fixed, the fan-out CONTROLLER SUBSHELL — as opposed to the `codex review` process it launches — is invisible to both pre-existing reapers by construction (one level of process hierarchy too high for the PGID reap, and the marker sweep's own documented environ-immutability limit for the recorded-descendant sweep). Observed 3× on the production fleet in one day: an orphaned parentless `autonomous-review.sh` fork plus a live `codex review` burning tokens against an already-decided PR, posting a second "Review findings:" comment under the same session trailer minutes after the authoritative verdict, with a loud rc-file write error in the log once the wrapper had deleted the fan-out dir.

**Producer**: `adapters/codex.sh::_run_codex_review` (layer 1's `rc -eq 124 || rc -ge 128` loop-break gate; layer 3's optional 5th-arg liveness gate). `lib-review-poll.sh::_reap_fanout_controller_subshells` (layer 2, new). `autonomous-review.sh` (the layer-2 call site fed `${_fanout_pids[@]:-}`, immediately after the existing `_reap_fanout_processes` / `_reap_fanout_recorded_descendants` calls; the layer-3 liveness-dir arg passed as `_FANOUT_DIR`; the layer-3 rc-sidecar write guard).

**Consumer**: the comment poller (unaffected — a terminal signal-death rc still routes through the existing INV-48 timeout-veto / `unavailable` sweep, unchanged for 124/137; 143 now joins that same sweep instead of triggering a fresh, uncounted run). `_classify_noverdict_agent` / `_aggregate_review_verdicts` ([INV-40](#inv-40-multi-agent-review-attribution-unanimous-aggregation-and-all-unavailable-fallback)) are untouched.

**Scope guards**: the codex **dev** path (`autonomous-dev.sh`) has no re-run controller — out of scope, nothing changed there. Session-report/label-flip teardown resilience is tracked separately (#402). Retroactive cleanup of any currently-orphaned fleet processes is an operator sweep, not part of this fix.

**Status**: **ENFORCED** (closes #406).

**Test**: `tests/unit/test-lib-review-codex.sh` — TC-CXRS-RUN-09/09b/10 (rc-143 regression: **proven to FAIL pre-fix** — the pre-#406 loop-break condition only checked `rc -eq 124 || rc -eq 137`, so a turn-1 143 fell through to the #209 re-run arm and fired a 2nd invocation; post-fix, exactly 1 invocation, rc 143 returned), TC-CXRS-RUN-11/12 (rc 137/124 byte-identical to the pre-existing RUN-07c/RUN-07), TC-CXRS-RUN-13/14 (rc 1 transient / INV-73 malformed-rc0 still re-run, unregressed), TC-CXRS-RUN-15 (rc 130 — an un-enumerated signal-death rc — also terminal, proving the fix is a genuine `rc >= 128` generalization, not an allowlist of `{124,137,143}`), TC-CXRS-RUN-16 (rc 2 clap-rejection still gated on the rc-2 arm, unaffected by the rc>=128 gate), TC-CXRS-LIVE-01..04 (the liveness-dir gate: removed-before-re-run → breaks with a log line and no 2nd invocation; present-throughout → unaffected; arg omitted → byte-identical no-op; removed mid-loop → breaks before the next launch). `tests/unit/test-codex-rerun-orphan-containment.sh` — TC-RFCS-001..008 (the new reaper: defined, no-op on empty/garbage args, a fixture controller subshell — `( sleep 60 ) &` with no setsid, mirroring the production shape exactly — is reaped by direct PID kill while **the calling "wrapper" stand-in survives** [the group-kill footgun guard], wrapper wiring calls the reaper with `_fanout_pids` and does NOT `lane_record_pgid` those PIDs, the two pre-existing reap call sites are byte-identical/unchanged), TC-RFCS-009 (the marker sweep call appears exactly twice, ordered first-sweep < controller-kill < second-sweep — closes the late-spawn race), TC-RSG-001..004 (the rc-sidecar write guard: dir-gone → silent no-write no-stderr; dir-present → unchanged; wrapper wiring gates the `printf` on `[[ -d "$_FANOUT_DIR" ]]`; the pre-existing missing-sidecar fallback is pinned as a regression guard), TC-E2E-406a..d (wrapper dry run: drives the REAL `_run_codex_review` + all three `lib-review-poll.sh` reapers against a real fixture `codex` process in the production call order — zero processes carry the run's fixture `ADT_FANOUT_LANE_MARKER` 5s after the reap, exactly one codex invocation, controller returns rc 143, no rc-sidecar stderr error; **proven to FAIL pre-fix** — reverting the loop-break condition reproduces 3 invocations). Test plan: `docs/test-cases/codex-rerun-orphan-containment.md`.

**Cross-references**:
- [INV-62](#inv-62-the-codex-review-lane-runs-the-codex-review-subcommand-auto-scoped-prompt-carried-gate-with-a-stdout-verdict-fallback) — sub-rule 3, whose terminal-rc set this invariant's layer 1 generalizes.
- [INV-43](#inv-43-command-mode-e2e-review-wait-budgets-must-not-be-smaller-than-the-e2e-they-dispatched) / [INV-104](#inv-104-the-inv-43-fan-out-reap-also-sweeps-recorded-descendants-that-re-parented-out-of-the-agents-process-group) — the two pre-existing reapers this invariant's layer 2 adds a third, structurally-distinct reaper alongside (neither pre-existing reaper reaches the controller subshell).
- [INV-48](#inv-48-per-side-review-wall-clock-timeout-agent_review_timeout-1h-default-with-browser-e2e-exclusion-and-timeout-veto) — the timeout-veto whose 124/137 rc-stickiness is preserved unchanged; 143 now routes through the same sweep instead of bypassing it.
- [INV-109](#inv-109-every-wrapper-run-mints-and-atomically-installs-a-durable-lane-registry-entry-before-spawning-any-background-child-including-token-daemons-and-heartbeat) — the durable lane registry / `lane_kill` delegate this invariant's layer 2 deliberately does NOT register the controller-subshell PIDs into.
- [`review-agent-flow.md` § Multi-agent fan-out (INV-40)](review-agent-flow.md#multi-agent-fan-out-inv-40) — the fan-out loop + reap call site this invariant amends.
...
## INV-114: every pipeline-initiated kill escalates TERM → bounded grace → SIGKILL, gated on group/scope emptiness, never on leader liveness alone

_Triage (issue #236): [machine-checked: tests/unit/test-lane-gc-p3-kill-paths.sh]_

**Rule** (Lane-GC series PR-3, design `docs/designs/lane-containment-gc.md` §4-C4, drafted as INV-106 in the design doc — renumbered here; see the numbering note below): every pipeline-initiated kill of a process group is SIGTERM first, then a bounded grace window, then SIGKILL **only if the group is still reachable at the end of that window**. Escalation and completion are gated on **group emptiness** (`kill -0 -- -PGID` failing), never on a single leader PID's liveness — a member that traps or ignores TERM can remain fully alive after its own session leader has already exited, and a leader-only gate would then skip the SIGKILL pass entirely, leaving that member to run forever. This closes RC2 from the design's forensic audit (`docs/designs/lane-containment-gc.md` §1): "Kill paths are TERM-only; KILL escalation exists only in `kill_stale_wrapper` and is gated on leader liveness."

Three kill sites carry this contract, sharing the escalation primitive `_kill_group_escalate <pgid> [grace_secs]` (`lib-lane.sh`):

1. **Wrapper TERM trap** (`install_agent_sigterm_trap` / `_agent_sigterm_handler`, `lib-agent.sh`): on receiving SIGTERM, the trap TERMs every pgid recorded in `${ADT_LANE_DIR}/pgids` — not just `_AGENT_RUN_PID` — closing the review-side dead arm: the review wrapper's agents run inside per-fan-out-member `( … ) &` subshells, so the MAIN shell's `_AGENT_RUN_PID` (where the trap fires) is always empty on the review side; only the durable registry reaches every spawn regardless of which shell spawned it. Each recorded pgid escalates via `_kill_group_escalate` in its OWN backgrounded job (5s grace) so N groups' grace windows run concurrently and the trap handler itself returns immediately — the wrapper's own EXIT-trap `cleanup()` must never be delayed waiting out an inline grace inside a signal handler. The pre-existing direct-children fallback (`pkill -TERM -P $$`, closes #109) runs **before** any escalator is backgrounded — see the ordering pin below.
2. **`kill_stale_wrapper` escalation gate** (`dispatch-local.sh`). The **legacy PID-file path**'s gate changes from a bare `kill -0 "$old_pid"` (leader-only) to `_pid_or_group_alive "$old_pid"`, which checks BOTH the individual pid and its negative-pid (whole-group) form — a TERM-trapping member whose leader already died now correctly triggers the KILL pass. The **pgrep-fallback orphan sweep** needs a different fix, because the pgrep-matched PID is a `nohup … &`-launched wrapper that is usually NOT a process-group leader (it inherits the spawning shell's pgid), so a bare group-form probe/kill against it is frequently `ESRCH`: the sweep instead BFS-walks each matched orphan's descendant tree at scan time (while it is still enumerable via `pgrep -P`), collecting every distinct REAL pgid (self-guarded — the walk never adds *our own* pgid to the group-kill set, so the sweep can never suicide its own process group) AND every distinct REAL pid, then TERM→grace→KILLs BOTH the collected pgid set (group-form) AND the collected pid set (individual-form). The individual-pid pass is required, not redundant, because a descendant spawned WITHOUT its own `setsid` inherits `dispatch-local.sh`'s OWN pgid — exactly the pgid the group-form pass must never signal — so only a per-pid kill reaches it (codex review round-4 [P1], reproduced empirically: a same-pgid, non-setsid TERM-trapping descendant survived the round-3 group-only walk with `wrapper=no child=yes`).
3. **`lane_kill`** (`lib-lane.sh`, the registry-authoritative escalation used by the `kill_stale_wrapper` delegate, `cleanup()`'s `lane_reap`, and future guardian/GC PRs): reaps every DISTINCT pgid recorded in a lane's `pgids` file, escalating each via `_kill_group_escalate` in its own backgrounded job (default 10s grace — the design's normative table keeps `kill_stale_wrapper`'s own 5s unchanged; `lane_kill`'s wider default reflects that it may be reaping a whole lane's worth of sub-lanes, not one known-stale PID). Serialized on the lane's own `reap.lock` (`flock -w 10`, best-effort — a missing/unwritable lock degrades to unlocked rather than blocking an otherwise-authorized kill) so a re-dispatch's delegate call and a concurrent graceful `cleanup()` can never issue overlapping KILLs against the same pgid.

**Ordering pin (found empirically while implementing this trap — a genuine bug, documented so it is never reintroduced):** the direct-children `pkill -TERM -P $$` fallback in the wrapper TERM trap MUST run BEFORE any `_kill_group_escalate … &` job is backgrounded, never after. Each backgrounded escalator is itself a direct child of the trap's own shell (`$$`); a `pkill -P $$` issued after backgrounding would TERM the escalator subshell mid-grace-wait and silently abort its KILL follow-through, letting the escalated group survive forever past its grace window — the exact failure the escalation contract exists to prevent. Verified by reproducing the bug with a real TERM-ignoring `setsid` child before the fix landed.

**Escalator process-group isolation (review rounds 5+8 [P1], both reproduced empirically):** EVERY backgrounded escalator — the wrapper TERM trap's per-pgid jobs (round 5) AND `lane_kill`'s per-pgid jobs (round 8) — runs in its own `setsid`-isolated session, never as a bare fork sharing the caller's pgid. A bare-forked escalator dies collaterally whenever the CALLER's own process group is later group-signalled mid-grace — and that happens routinely: `kill_stale_wrapper` escalates to a group-form SIGKILL against a still-alive wrapper whose `cleanup()`/`lane_reap` is running `lane_kill` at that very moment, and a second TERM/KILL to the wrapper's pgid is normal under the [INV-15] Step-5a race. SIGKILL cannot be trapped, so the only defense is being OUT of the reachable pgid entirely. `export -f _kill_group_escalate` carries the (call-free) primitive across the `setsid bash -c` boundary; the isolated jobs remain direct children, so `lane_kill`'s `wait` is unchanged; per-pgid concurrency is preserved. Degrades to the bare fork where `setsid` is unavailable (non-Linux). The pgrep-fallback sweep's TERM→KILL window is likewise pinned to the issue's 5s grace (an early-exiting liveness poll, not a flat sleep — round 8 [P1] finding 2).

**`pkill -P $$` scope pin (falsekill guard, grep-pinned):** the direct-children fallback is `-P $$` (this shell's direct children) and MUST NEVER be widened to `-f <script-name>` — a `-f` match would cross-kill sibling lanes sharing the same wrapper script name on a multi-issue/multi-project host. `tests/unit/test-lane-gc-p3-kill-paths.sh` grep-pins that no `pkill -f 'autonomous-'` (or any `-f`-based wrapper-script match) appears anywhere in the kill paths this invariant covers.

**Grace defaults** (design §12 R8, unchanged from the design's normative table): wrapper TERM trap 5s; `lane_kill` 10s; `kill_stale_wrapper` keeps its pre-existing 5s. `timeout --kill-after=30s` (INV-13) remains the in-process wall-clock backstop, unaffected by this invariant.

**Exit-code attribution** (extends [INV-26]): every path here is TERM-first, so a compliant process still exits 143 when it honors the first signal — attributed as dispatcher/self-induced per [INV-26], never counted against retry budget. **rc=137 produced by this escalation's bounded SIGKILL pass, following a pipeline-initiated TERM, is likewise self-induced, not a crash** — see the amended rule (1) wording in [INV-26].

**Numbering note**: the design doc (`docs/designs/lane-containment-gc.md` §8) drafted this as INV-106 at design time, when the documented head was INV-105. By this PR's open, INV-106 (provider conformance, #370) through INV-110 (universal lane tagging, #378/#392) had already shipped under those numbers via unrelated PRs. Per the design's own stated convention ("Renumber-on-rebase-collision: first-merged keeps, each INV-adding PR notes the convention") and the repo-wide INV numbering convention, this PR claims the first free slot, INV-114, instead.

**Producer**: `lib-lane.sh::_kill_group_escalate` (shared primitive); `lib-agent.sh::_agent_sigterm_handler`/`install_agent_sigterm_trap` (wrapper TERM trap); `dispatch-local.sh::_pid_or_group_alive`/`kill_stale_wrapper` (leader-or-group gate, both call sites); `lib-lane.sh::lane_kill` (registry-authoritative escalation, refactored onto the shared primitive from its PR-2 form).

**Consumer**: `dispatch-local.sh::kill_stale_wrapper`'s `lane_kill` delegate ([INV-109]); `autonomous-dev.sh`/`autonomous-review.sh`'s `cleanup()` via `lane_reap` ([INV-115]); a future guardian sidecar and `adt-gc.sh` (later Lane-GC PRs, not shipped here).

**Status**: **ENFORCED** (Lane-GC PR-3, closes #379).

**Test**: `tests/unit/test-lane-gc-p3-kill-paths.sh` — group whose leader died on TERM but a member traps TERM is KILLed within grace (both via direct `_kill_group_escalate` and via the full `install_agent_sigterm_trap` path against a registry pgid); `kill_stale_wrapper`'s leader-or-group gate fires the KILL pass against a TERM-trapping member whose leader already exited via the legacy PID-file path (TC-LGC3-023); the pgrep-fallback descendant-walk sweep reaps a `nohup`-launched (non-leader) wrapper AND a TERM-trapping child in its OWN `setsid` group (TC-LGC3-024) AND a TERM-trapping child that shares the wrapper's/dispatch-local.sh's OWN pgid with no `setsid` of its own (TC-LGC3-024b, the round-4 individual-pid-kill regression — verified to FAIL against the round-3 group-only walk); grep-pins that TERM precedes KILL at every kill site and that no `pkill -f 'autonomous-'` widening exists anywhere. `tests/e2e/run-lane-gc-p3-kill-paths-e2e.sh` (surfaced as its own CI job, not folded into the unit-test aggregate) drives the REAL, unmodified `dispatch-local.sh` CLI entry point against the same leader-dead/member-alive fixture shape through both the PID-file path and the pgrep-fallback sweep — the issue's stated fixture-tree E2E acceptance criterion. `tests/unit/test-kill-before-spawn.sh`, `tests/unit/test-dispatch-local-pgrep-type-scope.sh`, `tests/unit/test-pid-guard-pgid.sh`, `tests/unit/test-pid-alive-long-running.sh`, and `tests/unit/test-lib-lane.sh` stay green (extraction snippets updated to also capture the new `_pid_or_group_alive` sibling function; `lane_kill`'s external behavior — TERM→grace→KILL over every distinct recorded pgid — is unchanged by the internal refactor onto `_kill_group_escalate`).

**Cross-references**:
- [INV-26] (stall decision excludes dispatcher-induced terminations) — the rc=137/rc=143 attribution this invariant's escalation contract makes correct; amended in this PR to name the escalation paths explicitly.
- [INV-109] (lane registry) — `lane_kill`'s `pgids` file is this invariant's target set; the registry install this invariant's escalation reaps.
- [INV-110] (universal lane tagging) — the PGID-append chokepoint (`_run_with_timeout`) that makes every spawn reachable by the wrapper TERM trap's registry read.
- [INV-115] (bounded, ordered teardown) — the sibling invariant this PR ships alongside; `cleanup()`'s `lane_reap` call is the graceful-path consumer of `lane_kill`'s escalation.
- `docs/designs/lane-containment-gc.md` §4-C4 — the full kill-choreography design (normative signal table, all six kill paths) this invariant implements a subset of (the three pipeline-controlled paths; the guardian, GC, and systemd-scope paths are later PRs).

## INV-115: `cleanup()` reaps registry-recorded process groups before any network call, under a shared `reap.lock`, with every network call wall-clock bounded

_Triage (issue #236): [machine-checked: tests/unit/test-lane-gc-p3-kill-paths.sh]_

**Rule** (Lane-GC series PR-3, design §4-C4 row 2, drafted as INV-114 in the design doc — renumbered per the same collision noted under [INV-114]): both wrappers' `cleanup()` EXIT trap follows one fixed ordering, enforced identically on the dev and review sides:

1. **Reap first.** `lane_reap "$ADT_LANE_DIR" 5` (`lib-lane.sh`) acquires the lane's `reap.lock`, sets `STATE=reaping`, calls `lane_kill` (which itself re-acquires the same lock — a re-entrant no-op-wait within the same process, never a self-deadlock) to TERM→grace→KILL every registry-recorded pgid per [INV-114], then restores `STATE=cleaning`. This happens **before** the PID-file/heartbeat teardown and **before** any network call. The dev wrapper gains its **first-ever post-run reap** here (pre-PR-3, a dev-side agent CLI subtree that outlived the wrapper's own `wait` had no teardown-time reclamation at all); the review wrapper gains a **crash-path reap** — the existing graceful-verdict-path fan-out reap (`_reap_fanout_processes`/`_reap_fanout_recorded_descendants`, [INV-104]) is unchanged and still runs later in the main body after verdict resolution, but `lane_reap` in `cleanup()` now also covers the case that reap never reaches: a wrapper that dies (SIGTERM, SIGKILL, uncaught error) before the main body gets there, including mid-fan-out. The two reaps are idempotent with each other — a pgid the graceful path already reaped is simply already-dead by the time `lane_reap` runs.
2. **PID/registry state updates next.** The heartbeat PID is killed, `$PID_FILE`/`.heartbeat` are removed, and (best-effort, local-only) metrics/run-artifact writes happen — none of these touch the network.
3. **Network work last, every call bounded.** Every remaining GitHub-API call in `cleanup()` (`itp_post_comment`, `itp_transition_state`, `chp_pr_list`, `drain_agent_pr_create`, `drain_agent_bot_triggers`, `get_gh_app_token`, `emit_verdict_trailer`) routes through a `_teardown_call` closure that delegates to `_bounded_call 60 "$@"` (`lib-lane.sh`) — a 60-second wall-clock bound so a hung `gh` invocation can never leave the EXIT trap parked indefinitely while it still holds lane state. `_bounded_call` temporarily enables job control (`set -m`) around the background spawn (`"$@" > outfile 2> errfile &`, in the SAME shell — no nested `bash -c`, no `export -f`), then immediately restores the caller's prior monitor-mode setting once `$!` is captured (the child's pgid is fixed at fork time, so restoring right away never affects it, and a `set -euo pipefail` caller's shell options are never permanently altered). `set -m` gives the backgrounded call its own process group so a wrapped GitHub-API helper that itself forks a grandchild via command substitution (`out=$(curl …)`, the exact shape of `get_gh_app_token`) doesn't leak that grandchild past a plain (non-group) kill of the direct child on escalation (verified empirically: a wrapped function forking a grandchild survived a plain-kill implementation indefinitely; a group-form kill on escalation reaches it) — **and** every function already sourced into the caller's shell stays naturally visible to the wrapped call, since it runs in the same shell rather than a new process. (`export -f "$1"` + `setsid bash -c '"$@"' _ "$@"` was tried first and rejected: `export -f` exports only the ONE named function, but every real call site here internally calls OTHER functions — `itp_post_comment` → `itp_${ISSUE_PROVIDER}_post_comment` — that would NOT be exported, so the nested subshell failed with "command not found" on the first internal call, verified empirically.) The poll loop's liveness check inside the grace window uses a PLAIN (non-group) `kill -0 "$cpid"` — `set -m`'s pgid assignment is synchronous with `&` (no race, verified empirically over 500+ iterations), unlike the rejected `setsid` draft, whose async `setsid(2)` call could transiently ESRCH-miss a same-tick group probe. Output is captured to TWO SEPARATE private tmpfiles (never merged) and replayed to their own fds, so a caller's own `2>/dev/null`/`2>&1` on the `_bounded_call` invocation itself still works exactly as it would against an inline unbounded call — a single merged-stream implementation was tried first and rejected because several real call sites (e.g. `chp_pr_list … 2>/dev/null || echo "0"`) rely on their OWN outer redirection to drop stderr before using the value in a comparison, and a merged capture silently defeats that redirection. **`wait "$cpid"` is always `|| rc=$?`, never a bare statement followed by a separate `rc=$?` line**: `wait`'s own exit status mirrors the waited-on process's, so under a caller's `set -e` (every real `_teardown_call` site runs inside `set -euo pipefail`) a bare `wait` for a non-zero-exiting wrapped call would abort the CALLING shell at that exact line — before the intended `return "$rc"` ever executes — silently truncating `cleanup()` mid-teardown (verified empirically; not triggered by any current call site, all of which capture `_teardown_call`'s own result via `$(...)` or an explicit `||`, but load-bearing as a shared-utility invariant regardless). **Deliberately does not delegate to coreutils `timeout`**: every wrapped call site is a bash FUNCTION already sourced into the wrapper's shell, and `timeout <secs> <bash-function>` fails outright (`exec` cannot resolve a shell function as a binary) — this was verified empirically before choosing the background+poll implementation. Feature-detected/degrades per the design's own wording: if `_bounded_call` itself is unavailable (stale `lib-lane.sh` predating PR-3), `_teardown_call` falls back to an unwrapped call — a slower teardown on a hang, never a silently dropped call.

The FIFO clean-exit handshake the design's normative table also lists in this row is **feature-guarded as a no-op in this PR** — the guardian sidecar and its `guard.fifo` ship in a later Lane-GC PR (design §9 PR-5); there is no FIFO to signal yet. When PR-5 lands the FIFO, `cleanup()` gains the `printf 'done\n' >&$ADT_GUARD_FD; exec {ADT_GUARD_FD}>&-` handshake between steps 1 and 2 above (reap-first, then signal-before-network, matching the design's ordering exactly) — this invariant's numbered ordering already reserves that slot so PR-5 is a pure addition, not a re-ordering.

**Why**: pre-PR-3, `cleanup()`'s teardown order was PID-file-first with no bounded network calls and no dev-side reap at all — a hung `gh api` call inside `cleanup()` could park the EXIT trap indefinitely while `STATE` sat at whatever PR-2 last wrote, and a dev-side agent subtree that outlived its own `wait` was invisible to any teardown-time mechanism. Reap-first with a bounded lock, followed by bounded network calls, means `cleanup()` now has a provable worst-case wall-clock (grace + N × 60s) instead of an unbounded one, and no dev-side lane ever exits without at least one reap attempt.

**Producer**: `lib-lane.sh::lane_reap` (the reap-first primitive); `lib-lane.sh::_bounded_call` (the network-call bound); `autonomous-dev.sh`/`autonomous-review.sh::cleanup()` (the ordering + the `_teardown_call` wrapping at every network call site).

**Consumer**: none outside the wrapper's own EXIT trap in this PR — a future guardian/GC PR reads the `STATE=cleaning`→`STATE=clean-exit` transition this ordering produces, same as [INV-109] already documents.

**Status**: **ENFORCED** (Lane-GC PR-3, closes #379).

**Test**: `tests/unit/test-lane-gc-p3-kill-paths.sh` — `cleanup()` with a hung `gh`-shaped stub function completes within the bounded budget (well under 90s) and still flips labels; `_bounded_call` against a fast function preserves stdout/stderr/exit-code exactly; `_bounded_call` against a hanging function is terminated at its bound and returns 124; `_bounded_call` against a function that forks a grandchild via command substitution leaves no orphaned grandchild after escalation (the `set -m`+group-kill regression); a caller's own `2>/dev/null` on the `_bounded_call` invocation still drops the wrapped function's stderr (the dual-tmpfile-stream regression); `_bounded_call` called BARE (no `||`/`if`/`$(...)` guard) under `set -euo pipefail` against a non-zero-exiting wrapped function propagates that exit code as ITS OWN return rather than aborting the calling shell mid-`wait` (the bare-`wait`-under-`set -e` regression); `lane_reap` transitions `STATE` live→reaping→cleaning and reaps a live recorded pgid; concurrent `lane_kill`/`lane_reap` calls contending on the same `reap.lock` serialize rather than double-KILL the same pgid (verified via a shared counter fixture, not `strace` — no `strace` dependency introduced); grep-pins that `cleanup()`'s reap call precedes its first network-work call site in both wrapper files.

**Cross-references**:
- [INV-114] (kill-escalation contract) — `lane_reap`'s call into `lane_kill` is this invariant's reap step; INV-114 defines HOW that reap escalates, this invariant defines WHEN it runs relative to everything else in `cleanup()`.
- [INV-109] (lane registry, `STATE` lifecycle) — the `STATE=cleaning`/`STATE=clean-exit` transitions this ordering produces; PR-2 wired the markers, this PR wires the actual reap-first behavior between them.
- [INV-104] (fan-out reap) — the review side's pre-existing graceful-path reap, unchanged by this PR; `lane_reap` in `cleanup()` is the crash-path complement, not a replacement.
- [INV-13] (`AGENT_TIMEOUT` wall-clock bound) — the analogous bound on the agent CLI's own runtime; this invariant is the teardown-path analog for network calls.
- `docs/designs/lane-containment-gc.md` §4-C4 row 2, §12 R10 — the full teardown-ordering design (including the FIFO handshake this PR feature-guards until PR-5).
...
## INV-117: a periodic, box-wide GC reclaims dead-lane process residue under a decision table that fails toward leak, never toward false-kill — dry-run by default

_Triage (issue #236): [machine-checked: tests/unit/test-lane-gc-p4-gc.sh]_

**Rule** (Lane-GC series PR-4, design `docs/designs/lane-containment-gc.md` §4-C5/§6, drafted as INV-110 in the design doc — renumbered here; see the numbering note below): `adt-gc.sh` is a new, issue-independent entry-point script that scans every project's lane registry under `${ADT_STATE_ROOT}/autonomous-*/lanes/` in one invocation and reclaims process residue a dead lane left behind. It is a flock singleton (`$ADT_STATE_ROOT/adt-gc.lock`) and runs in one of two modes: `--dry-run` (default — classifies and logs every candidate, signals nothing) or `--kill` (`ADT_GC_ENFORCE=1` is the env equivalent). Two additional flags: `--quick` (Pass 1 only — no env reads, no same-uid process enumeration; meant for the opportunistic call `dispatch-local.sh` now makes at the top of every dispatch, `adt-gc.sh --quick || true`) and `--doctor` (read-only health report: `flock`/`setsid` presence, timer installation, macOS `python3`/linger status, `ADT_STATE_ROOT` content).

**The four-pass decision table (design §6, normative)** — every rule fails toward *skip* (leak) rather than toward a false kill (design principle 5):

1. **Pass 1 — registry-driven** (no env reads; the only pass `--quick` runs). Consults `lane_probe`/`lane_get` alone: a `live` lane is always skipped (rule 1.1); a `dead` lane whose guardian is alive and `STATE=reaping` for under 5 minutes is left to the guardian (rule 1.2 — tightened from the design's rev-2 15-minute draft); a dead-and-wedged lane (guardian dead, or `reaping`/`cleaning` beyond that bound, or lane age > 600s) is reaped via `lane_kill` and marked `STATE=gc-reaped` (rule 1.3); a lane in a terminal state (`clean-exit`/`reaped-by-guardian`/`gc-reaped`) or an unparseable lane file, once its age exceeds 24h, is collected — but a live `GUARDIAN_PID` is TERMed (1s grace, then KILLed if still alive) **before** the directory is `rm -rf`'d, so GC never yanks a directory out from under a guardian still holding `guard.fifo` open (rule 1.4); anything younger than 24h or still-unparseable-but-fresh is a skip+WARN (rule 1.5). A surviving `.pending-<id>/` mid-crash-mint orphan is collected by the same 24h/guardian-first rule.
2. **Pass 2 — tagged-orphan sweep** (same-uid process enumeration; `ps -eo pid= -U "$(id -u)" | grep -vw "$$"` — the self-match-safety line the design's own attack ledger calls out). A candidate is kill-eligible **only when ALL** of: (a) its env carries `ADT_LANE_ID=<X>` where lane X resolves DEAD via Pass 1's own rules with a 300s age floor, **or** the legacy signature `AUTONOMOUS_CONF_LOADED_FROM` present **AND** `CC_USER=autonomous-{dev,review}-bot` **AND** `ppid==1`, with a 600s floor (the `∧`, never `∨` — a bare conf-sourcing in an operator's own SSM debug shell must never authorize a kill); an `ADT_LANE_ID` absent from every registry is an unconditional skip (`unknown_class`), never treated as legacy; (b) its env is READABLE at all, and, once read, carries no `TERM_PROGRAM` (an UNREADABLE env — process died mid-scan, EPERM, macOS with no procargs2 shim — is UNKNOWABLE, not "known clean", and is an unconditional skip in its own right, checked separately from and before the `TERM_PROGRAM` absence test; operator tooling and every unknowable-env process alike are unconditionally untouchable); (c) its pgid contains no live-wrapper argv match (`pgrep -g $PG -f 'autonomous-(dev|review)\.sh'`, survives comm truncation and a launcher-bridge exec chain); (d) its pgid is outside every live lane's recorded `pgids`, outside every live PID-file's pgid, and outside the descendant ancestry of every live wrapper process **box-wide** (not scoped to one `PROJECT_DIR` — see the design-note interpretation below); (e) process age clears the applicable floor; (f) its pgid is itself a SAFE kill target (numeric, `>1`, not GC's own process group) — GC's own kill-primitive call sites refuse pgid 0 (a kernel alias for the SENDER's own group, not any candidate's), pgid 1, and GC's own group unconditionally, falling back to an individually-pid-gated TERM→KILL when the pgid is unsafe. Escalation is TERM → 10s grace → KILL (group-form via the shared `_kill_group_escalate`, individual-pid fallback, each gated by its own last-line-of-defense pid/pgid-safety check). Banned as kill keys, full stop: `CLAUDE_CODE_SESSION_ID`, kernel sid liveness, a bare `ppid==1` on its own, and any comm/process-name match.

   Conjuncts (b), (c), (d), (e), and (f) above are encoded in ONE shared `_gc_common_kill_guards <pid> <pg> <age_floor> [rule_id]` function every Pass-2 AND Pass-3 kill-authorization site calls — see Pass 3 below for why this exists (review round-2, P1-1/P1-4/P2-1).
3. **Pass 3 — env-blind classes**, each now running through the SAME `_gc_common_kill_guards` shared function Pass 2 uses (review round-2 fix — pre-fix, each Pass-3 sub-rule applied an ARBITRARY SUBSET of Pass 2's guard set, and rule 3.4 applied NONE of it at all): a Chrome process whose `--user-data-dir` argv matches a dead lane's recorded `CHROME_PROFILE_HINT`, age floor 0 — an exact structural match needs no wait (3.1); a heuristic Chrome match (`/tmp/puppeteer_dev_chrome_profile-*`, `ppid==1`, age > 2h) PLUS two rule-local extra conjuncts kept outside the shared function since no other rule needs them — no live process shares that `--user-data-dir` profile, and no live `chrome-devtools-mcp`-argv-matching ancestor (3.2); a wedged `gh pr checks --watch`/`gh api` whose `GH_TOKEN_FILE` directory no longer exists on disk, age floor 300s (inherits rule 2.1's exact-join floor, not the legacy arm's 600s — same exact-positive-signal confidence class) (3.3); an E2E server whose recorded lane `WORKTREE` no longer exists and whose `cwd` was under it, age floor 0 (3.4). Crashpad/helper processes are never judged by name — only by falling through to Pass 2's env-based join, per the design's explicit "judged by intact env via Pass 2, never by ppid/name" rule (3.5).
4. **Pass 4 — live-lane sustained-CPU alert, flag-only, never a kill path**: a pipeline-tagged process recorded in a **live** lane's `pgids`, with `cwd` under a `.worktrees/*/hooks/`-shaped path and `%cpu > 80` observed on **two consecutive** `adt-gc.sh` invocations, emits one `LIVE_BURNER_ALERT lane=<dir> pid=<n> argv=<head> cpu_pct=<n> age_s=<n>` log line and nothing else — the design forbids killing anything under a live lane (principle 5; RC6's root cause was a live-agent CPU burner, not a dead one). Cross-tick "was this already high" state lives in a small flat file (`$ADT_STATE_ROOT/adt-gc-pass4.state`) — not the lane registry, since this is flag-only telemetry with no atomicity/durability requirement.

**Log discipline + metrics (design §4-C5).** Every kill/would-kill/skip decision is appended to one box-wide `$ADT_STATE_ROOT/adt-gc.log`, self-rotated at 25MB (single-generation `.1`, mode 600 preserved — mirrors [INV-68]'s dispatch-local.sh log-retention pattern, never touching per-lane `guardian.log`). Every run ends with one summary line, `ADT_GC_SUMMARY skips=<n> would_kill=<n> killed=<n> would_kill_legacy_signature=<n> unknown_class=<n> live_burner_alerts=<n> elapsed_ms=<n>`, both echoed to stdout and appended to the log; a best-effort `metrics_emit adt_gc_summary …` call (guarded — a missing/stale `lib-metrics.sh` degrades to no metrics, never an abort) feeds the same fields into the existing [INV-70] observe-only metrics lane, one event per project the run touched.

**`install-gc-timer.sh`** (a second new entry-point script, shipped in the same PR) is the idempotent per-**host** installer — one timer per box, never per project, since `adt-gc.sh` itself already scans every project's registry in one invocation. On Linux it edits the invoking user's crontab (`*/10 * * * *`), guarded by a fixed marker comment so a re-run **replaces** the entry (never stacks a duplicate) and `--uninstall` cleanly removes exactly that line while preserving unrelated crontab content — cron is chosen over a systemd `--user` timer because it needs no `loginctl enable-linger`. On macOS it writes `~/Library/LaunchAgents/com.adt.lane-gc.plist` (`StartInterval=600`) and `launchctl bootstrap`s it into `gui/$(id -u)` (bootout-then-bootstrap on every install for idempotent reload) — cron is deliberately avoided there because scripts run from cron trip TCC permission prompts on macOS.

**macOS env-read authorization (design §7 platform:F2).** `ps eww` is banned for kill authorization on macOS — its output has no argv/env delimiter, so a value containing a literal space is indistinguishable from a new argv element. `lib-lane.sh` gains a `sysctl kern.procargs2` shim (`_procargs2_py`, invoked via `python3`) for this PR: the KERN_PROCARGS2 buffer is argc-delimited (a leading int32 argc, then NUL-terminated `exec_path`/`argv[]`/`envp[]`), so it is the only portable way to split argv from envp on Darwin without guessing. The shim is split into a pure parser (unit-testable on Linux CI against a synthetic buffer — no macOS runner or real `sysctl` call required) and a live ctypes-based fetcher. Absent `python3`, macOS GC degrades to registry-authoritative only (Pass 1 alone; Pass 2/3's env-tag reads become unavailable and any env-derived signal is a dry-run diagnostic, never a kill authorization) — `--doctor` surfaces this as a `[WARN]`.

**New `lib-lane.sh` portability primitives** (design §12 R13: helpers live in the lib, `adt-gc.sh` stays a thin entry-point): `proc_argv` (Linux `/proc/PID/cmdline`, macOS via the procargs2 shim), `proc_ppid` (Linux `/proc/PID/stat` field 4, macOS/BSD `ps -o ppid=`), `proc_pgid` (`ps -o pgid=`, identical both platforms), `env_lookup` (an exact-key convenience wrapper over the existing `env_of`), and (review round-2) `env_readable` — true iff a process's environment can actually be READ at all right now, distinct from `env_of` returning empty because a readable env genuinely lacks a key. None of these change any existing function's behavior.

**Review round-2 hardening (4 P1 + 5 P2 from an independent review).** Two classes: (A) every Pass-3 sub-rule applied an arbitrary subset of Pass 2's guard set instead of the full one — fixed by extracting `_gc_common_kill_guards`, called by every Pass-2/3 kill-authorization site (see Pass 2/3 above). (B) protective checks failed OPEN — `_gc_has_term_program` returning false for an unreadable env was indistinguishable from "TERM_PROGRAM genuinely absent", and `_gc_kill_candidate` accepted pgid 0/1/GC's-own-group or pid `$$` as kill targets — fixed with `env_readable`/`_gc_env_unknowable` (checked first, fail toward leak) and `_gc_safe_kill_pid`/`_gc_safe_kill_pgid` gates at every kill-primitive call site. Regression-pinned by `tests/unit/test-lane-gc-p4-gc.sh` TC-LGC4-200..214.

**Review rounds 3-4 (subsequent verification passes over round-2's own fixes).** Round 3 found `env_of`/`env_readable` had drifted out of source-alignment on Darwin (readable-but-empty re-opened the P1-2 hole one layer down — `env_of` now shares the same procargs2 source), and that the timer installer's single-quoted cron paths didn't reject an embedded single quote (shell token injection — `'` now joins the `%`/newline reject set). Round 4 re-reviewed the round-2 guard functions themselves and found `_gc_safe_kill_pid` didn't reject pid 0 (a kernel alias for the sender's own process group, same as pgid 0) and `_gc_safe_kill_pgid` failed OPEN when GC's own pgid was unreadable (treated "unknowable" as "safe" instead of refusing). Regression-pinned by TC-LGC4-215/216 (round 3) and TC-LGC4-217/218 (round 4). Full rationale for all four rounds: `docs/designs/lane-gc-p4-adt-gc.md`'s review interpretation notes.

**Interpretation notes** (decisions made at a degree of freedom the parent design leaves open — recorded in `docs/designs/lane-gc-p4-adt-gc.md`, "pick the simpler, more maintainable option" per the autonomous-dev skill's decision-making guidelines): rule 2's live-wrapper-ancestry protection is scoped **box-wide**, not per-`PROJECT_DIR` — `adt-gc.sh` has no central manifest of every onboarded project's `PROJECT_DIR` to scope the walk to, and a box-wide walk can only cause GC to skip a candidate it would otherwise have swept, never cause a false kill, so it is strictly the more conservative choice. Rule 3.1's "dead lane's `LANE_SCRATCH`" is implemented against the registry's existing `CHROME_PROFILE_HINT` field (already shipped by PR-2 for exactly this purpose) rather than inventing a new `LANE_SCRATCH` registry key.

**Numbering note**: the design doc (`docs/designs/lane-containment-gc.md` §8) drafted this as INV-110 at design time. By this PR's open, PR-2 (#378) had already shipped INV-109/INV-110 and PR-3 (#379/#405) had shipped INV-114/INV-115 (each already renumbered once from the design's own draft — see `docs/designs/lane-gc-p3-kill-paths.md`). Current head at this PR's open was INV-115, so this PR first claimed **INV-116**. A rebase onto `origin/main` then pulled in #422's GitLab transport invariant, which had independently (and earlier-mergedly) claimed INV-116 for itself — a second renumber-on-collision, per the same repo-wide convention ("first-merged keeps it"): this PR yields the slot and claims the next free one, **INV-117**.

**Producer**: `skills/autonomous-dispatcher/scripts/adt-gc.sh` (all four passes); `skills/autonomous-dispatcher/scripts/install-gc-timer.sh` (the timer installer); `lib-lane.sh::proc_argv`/`proc_ppid`/`proc_pgid`/`env_lookup`/`_procargs2_py` (the new portability primitives this PR adds); `dispatch-local.sh` (the opportunistic `adt-gc.sh --quick || true` call).

**Consumer**: the INV-67/[INV-70] metrics collector (via `ADT_GC_SUMMARY`/`adt_gc_summary`); the eventual PR-8 enforcement-flip decision, which the design gates on a ≥2-week soak showing zero `would_kill_legacy_signature` and a stable `unknown_class`; a future guardian sidecar (PR-5, not shipped here) whose `STATE=reaping` window this PR's rule 1.2 already respects.

**Status**: **ENFORCED (dry-run default)** — Lane-GC PR-4, closes #380. The `--dry-run` → `--kill` default flip is explicitly **out of scope** here (design §9 PR-8; requires the soak sign-off above) — this PR ships full `--kill` capability (opt-in via `--kill`/`ADT_GC_ENFORCE=1`) but never flips the *default*.

**Test**: `tests/unit/test-lane-gc-p4-gc.sh` — fixture harness spawning real (never simulated) tagged/legacy/decoy processes classifies each exactly per §6 with zero false positives (tagged sleep → would-kill; legacy-sig-with-CC_USER → would-kill-legacy; legacy-sig-WITHOUT-CC_USER decoy → skip; `TERM_PROGRAM` decoy → skip; unknown-lane-id decoy → skip [`unknown_class`]; live-lane daemon → skip; group-member-has-live-wrapper → skip via rule 2.3; crashpad-shaped-but-env-tagged decoy → judged like any Pass-2 candidate, never by name); Pass 1's full rule table (1.1 through 1.5, plus the F3 arithmetic-note regression that a `STATE=reaping` lane under the 600s floor is still rule-1.3-eligible); rule 1.4's guardian-TERMed-before-`rm -rf` ordering; Pass 3's four env-blind classes; Pass 4's two-consecutive-tick gate and its proof that the flagged process survives (never killed); the flock singleton under concurrent invocation and `--quick`'s `flock -w 3` starvation-avoidance behavior (waits and completes, never bails like the full run's `-n`); `--quick` touching Pass 1 only; sub-1-second `--quick` against 50 lane dirs; 25MB→26MB log rotation; the `ADT_GC_SUMMARY` line's exact field set; `--doctor`'s WARN/FAIL reporting including a `flock`-absent fixture; `install-gc-timer.sh`'s idempotent crontab edit/uninstall and the macOS launchd plist install/reload/uninstall cycle (`_LANE_UNAME_OVERRIDE` test seam); the new `proc_ppid`/`proc_pgid`/`proc_argv`/`env_lookup` primitives against real spawned processes; the `_procargs2_py` pure-parser path against a synthetic procargs2 buffer (macOS ACs deferred per the design's own §11 carve-out where no macOS CI runner exists — this parser-level test is the required Linux-CI-runnable proof); grep-pins for the banned-key list and the always-conjoined legacy-signature check; `dispatch-local.sh`'s opportunistic `--quick` call ordering and its `|| true` graceful degradation when `adt-gc.sh` is absent.

**Cross-references**:
- [INV-109] (lane registry) — every Pass-1/2/3 predicate reads `lane_probe`/`lane_get`/`pgids` from this registry; GC never invents its own liveness signal.
- [INV-110] (universal lane tagging) — the `ADT_LANE_ID` env tag Pass 2's exact-join arm keys on.
- [INV-114] (kill-escalation contract) — every GC kill (Pass 1 rule 1.3 via `lane_kill`, Pass 2/3 via `_kill_group_escalate`) reuses the same TERM→grace→KILL primitive, never a bespoke escalation.
- [INV-68] (single-generation log rotation) — the pattern `adt-gc.log`'s own rotation mirrors.
- [INV-70] (observe-only metrics) — `ADT_GC_SUMMARY`'s consumer contract.
- `docs/designs/lane-containment-gc.md` §4-C5, §6, §7, §9 PR-4 — the full periodic-GC design (decision table, platform matrix, rollout plan) this invariant implements; `docs/designs/lane-gc-p4-adt-gc.md` — this PR's numbering re-verification and interpretation notes.
