...
**Verdict-comment authentication (#389)**: `BOT_LOGIN` is never set in the dispatcher's own process (it is resolved only inside `autonomous-review.sh`'s separate process), and the historical `FALLBACK_SESSION_ID` binding is never assigned anywhere — so in every real deployment the helper runs with no actor signal. In that state it authenticates the verdict candidate **structurally**, the same way the [INV-105](#inv-105-a-non-converging-devreview-loop-a-failed-substantivedev-actionabletrue-verdict-that-churns-n-completed-zero-commit-dev-resume-rounds-against-a-frozen-pr-head-is-detected-and-halted--the-breaker-transitions-to-stalled-then-posts-one-structured-reasonnon-convergence-report-gated-behind-the-shared-may_stall_now-live-pid-pre-gate-idempotent-per-issue-head-trailer-hash-session_id) convergence breaker does: a comment qualifies only if its *entire body* is a bare `<!-- review-verdict: ... -->` trailer (anchored end-to-end match; `emit_verdict_trailer` posts the trailer as its own bare comment). Prose-embedded or prose-suffixed trailers are rejected (forgery posture identical to the breaker's round-14 anchor). The accepted residual — a human posting a byte-for-byte bare trailer as their whole comment — matches the documented residual at the breaker's call sites. Under the structural branch the legacy no-trailer→`failed-substantive` fallback is unreachable by design (prose comments are not candidates), so a genuinely verdict-less completed session still takes the INV-12 operator-handoff park. Pre-#389, the no-signal branch refused to classify entirely, which parked **every** completed-session `pending-dev` issue at INV-12 the moment the remote session-completed probe started working (#369) — observed fleet-wide on 2026-07-03.
