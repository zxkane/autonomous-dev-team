...
3. **Network work last, every call bounded.** Every remaining GitHub-API call in `cleanup()` (`itp_post_comment`, `itp_transition_state`, `chp_pr_list`, `drain_agent_pr_create`, `drain_agent_bot_triggers`, `get_gh_app_token`, `emit_verdict_trailer`) routes through a `_teardown_call` closure that delegates to `_bounded_call 60 "$@"` (`lib-lane.sh`) — a 60-second wall-clock bound so a hung `gh` invocation can never leave the EXIT trap parked indefinitely while it still holds lane state. `_bounded_call` `export -f`'s the target (a no-op when it's a real binary) and backgrounds it via `setsid bash -c '"$@"' _ "$@"` — spawning the wrapped call as its own session/process-group leader, exactly like `_run_with_timeout`'s agent spawn — because a wrapped GitHub-API helper that itself forks a grandchild via command substitution (`out=$(curl …)`, the exact shape of `get_gh_app_token`) would otherwise leak that grandchild past a plain (non-group) kill of the direct child (verified empirically: a wrapped function forking a grandchild survived a plain-kill implementation indefinitely; `setsid` + a group-form kill on escalation reaches it). The poll loop itself uses a PLAIN (non-group) `kill -0 "$cpid"` liveness check, never the group form — the group form can transiently ESRCH-miss in the brief window before `setsid()` actually completes, which would misclassify a just-spawned call as already-exited and `wait` on it for the FULL duration instead of returning at the bound (also verified empirically); by escalation time several seconds have elapsed, so the group form is safe there. Output is captured to TWO SEPARATE private tmpfiles (never merged) and replayed to their own fds, so a caller's own `2>/dev/null`/`2>&1` on the `_bounded_call` invocation itself still works exactly as it would against an inline unbounded call — a single merged-stream implementation was tried first and rejected because several real call sites (e.g. `chp_pr_list … 2>/dev/null || echo "0"`) rely on their OWN outer redirection to drop stderr before using the value in a comparison, and a merged capture silently defeats that redirection. **Deliberately does not delegate to coreutils `timeout`**: every wrapped call site is a bash FUNCTION already sourced into the wrapper's shell, and `timeout <secs> <bash-function>` fails outright (`exec` cannot resolve a shell function as a binary) — this was verified empirically before choosing the background+poll implementation. Feature-detected/degrades per the design's own wording: if `_bounded_call` itself is unavailable (stale `lib-lane.sh` predating PR-3), `_teardown_call` falls back to an unwrapped call — a slower teardown on a hang, never a silently dropped call.
