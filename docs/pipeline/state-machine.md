...
stateDiagram-v2
    ...
    reviewing --> pending_review: PASS but a mandatory REVIEW_BOTS review is still missing — re-queue for re-review (INV-78, scoped mode; bounded by BOT_REVIEW_WAIT_MAX)
    reviewing --> pending_dev: Mandatory bot review absent after BOT_REVIEW_WAIT_MAX waits — substantive FAIL (INV-78)
