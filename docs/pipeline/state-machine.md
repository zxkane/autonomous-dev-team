...
- **Step 5a SIGTERM path**: now **convergent** ([INV-15](invariants.md#inv-15-step-5a-sigterm-race-is-non-deterministic), fixed in PR-6). Dispatcher sends `kill $PID` (SIGTERM), then edits labels to `pending-review`. The wrapper's `trap on_sigterm TERM` sets `RECEIVED_SIGTERM=1` and forwards the signal to descendants via `pkill -TERM -P $$` so the agent CLI exits promptly. The wrapper's `cleanup()` rewrites `exit_code 143 → 0` when `RECEIVED_SIGTERM=1 && PR_EXISTS>0`, then routes through the success branch to `+pending-review`. Both writers now agree on the target.
