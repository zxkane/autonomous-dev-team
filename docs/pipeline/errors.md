...
| `ADT_CFG_AGENT_BINARY_MISSING` | The configured agent CLI binary is not on `PATH` | `AGENT_CMD` resolves to a launch binary (`AGENT_CMD` itself, or `kiro-cli` when `AGENT_CMD=kiro`) that `command -v` cannot find on the execution host's `PATH` — preflighted before `run_agent`/`resume_agent` (`lib-agent.sh`) **and** before the codex review lane (`lib-review-codex.sh::_run_codex_review`, which launches `codex review` directly), so a missing binary surfaces an envelope instead of an opaque rc 127 / generic session failure / dropped-`unavailable` review agent. **[#458]** Before concluding the binary is genuinely absent, `preflight_agent_binary` probes a fixed list of common user-level install dirs (`~/.local/bin`, `~/bin`, `~/.npm-global/bin`, nvm shim dirs) via `_probe_user_install_dirs` — this same code fires either way, but the `cause`/`remediation` text differs by branch (see next column) | **Found in a probed dir** (binary installed, just not on this non-login shell's `PATH`): cause names the found path; remediation is PATH-specific — extend `PATH` in the dispatcher/wrapper environment, use an `AGENT_LAUNCHER` that sources the user profile, or set `AGENT_CMD` to the absolute path, then re-dispatch. **Not found anywhere**: unchanged install-focused remediation, with the effective `$PATH` now included in the cause for diagnosis | issue-comment / dispatcher-alert |
...
| `ADT_CFG_TIMEOUT_TOOL_MISSING` | Neither `timeout` nor `gtimeout` is on `PATH` on the host that sources `lib-agent.sh` | [INV-126](invariants.md#inv-126-fail-closed-when-no-wall-clock-timeout-binary-is-available): `AGENT_TIMEOUT`/`AGENT_REVIEW_TIMEOUT` (INV-13) is the ONLY wall-clock bound on an agent run (no `--max-turns`, no token gate); with neither binary present the wrapper refuses to launch rather than run unbounded. Detected once at `lib-agent.sh` SOURCE TIME — the execution host under every `EXECUTION_BACKEND` (`local` and `remote-aws-ssm`), never `dispatcher-tick.sh` | Install coreutils so `timeout`/`gtimeout` is on `PATH` (macOS: `brew install coreutils`), or opt into the pure-shell PGID watchdog fallback via `AGENT_TIMEOUT_WATCHDOG_FALLBACK=true` in `autonomous.conf`, then re-dispatch | issue-comment / dispatcher-alert |
