...
| [`adapter-spec.md`](adapter-spec.md) | **Normative** agent-CLI adapter contract (`spec_version: 1`): the `invoke(mode, …) → AdapterResult` interface with a mode axis, the four-axis result, the verdict-artifact / fixture-manifest / error-envelope JSON Schemas under [`schemas/`](schemas/), and the per-CLI mapping appendix (INV-63). Later adapter work implements it. |
