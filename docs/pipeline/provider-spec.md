...
|---|---|---|
| `itp_list_by_state STATE LABELS_AND_CSV LIMIT FIELDS_CSV` | `list_new_issues` (`lib-dispatch.sh:47`), `list_pending_review` (`:73`), `list_pending_dev` (`:91`), `list_stale_candidates` (`:110`) | **ABSTRACT contract ([W1a], #371) — no gh flags and no jq programs cross the seam.** `STATE` ∈ `open\|closed\|all`. `LABELS_AND_CSV` = comma-separated label names combined with AND semantics, empty = no label filter. `LIMIT` = positive int, applied **server-side** to the AND-labels-filtered candidate set **before any caller-side subtraction** (same pipeline point as the pre-W1a `gh --limit`). `FIELDS_CSV` ⊆ `number,title,labels,assignees,comments` (`assignees` added #435, ISSUE_FILTER seam PR-A). Returns a JSON array of objects with EXACTLY the requested fields, normalized: `number` (int), `title` (string), `labels` (array of label-NAME strings — not `{name}` objects), `assignees` (array of assignee-LOGIN strings — not object shapes; `[]` when unassigned, never null; capability-gated by `.caps` `assignees` — §4.1), `comments` (the [INV-90] normalized comment array, ascending by `createdAt`). Canonical sort: `number` ascending — the leaf pins ONE order regardless of `gh`'s default. No matches → `[]` (never null/empty string). **MUST return the full set** (provider walks pagination internally — see §3.5). Caller-side predicates (the [INV-25] terminal-state subtraction, negation) are re-derived by the caller by filtering the returned normalized array — they do NOT travel as a jq program. **Asserted** (shape/sort/fail-closed) by `tests/provider-conformance/run-provider-conformance.sh` (#370, R6; `assignees` exact-key assertions #435). |
...
| `itp_list_forbidden_combos STATE LABELS_AND_CSV LIMIT` | `list_hygiene_residue` (`:143`) | **ABSTRACT contract ([W1a], #371).** Same enumeration point as `itp_list_by_state`. Returns the normalized array shape with fields `number,labels,assignees` (widened from `number,labels` #435, ISSUE_FILTER seam PR-A — this verb takes no `FIELDS_CSV`, so `assignees` is added UNCONDITIONALLY; `[]` when unassigned, never null), already filtered to the **[INV-25] forbidden label combination** — the LEAF owns the combo filter (server-side-optimizable for providers with query languages): terminal set = `{approved, stalled}`; transitional set = `{in-progress, reviewing, pending-review, pending-dev}`; forbidden = terminal AND transitional. This is the ONE deliberate exception to "predicates stay caller-side" — `list_hygiene_residue` is now a thin pass-through. Distinct verb because a single `STATE` set cannot express an intersection-of-incompatible-states query. **[M3]** **Asserted** by `tests/provider-conformance/run-provider-conformance.sh` (#370, R6; widened-shape exact-key assertion #435). |
...
| `itp_read_task ISSUE FIELDS_CSV` | `check_deps_resolved` (`lib-dispatch.sh:480`), `autonomous-dev.sh:963`/`:1282`, `autonomous-review.sh:3611`, `status.sh:87`, `mark-issue-checkbox.sh:95`, `issue_mention_login` ([INV-138], `lib-issue-provider.sh`) | **ABSTRACT contract ([W1b], #396) — no gh flags and no jq programs cross the seam.** `FIELDS_CSV` ⊆ `title,body,state,labels,comments,author`. Returns a single JSON object with EXACTLY the requested fields, normalized: `title` (string), `body` (string; absent body → `""`), `state` (provider-neutral UPPERCASE `OPEN`\|`CLOSED` — deliberately matches GitHub's raw tokens so `status.sh`'s `_next_action` gate (`[[ "$ISSUE_STATE" != "OPEN" ]]`) ships byte-unchanged), `labels` (array of label-NAME strings — not `{name}` objects), `comments` (the [INV-90] normalized comment array, ascending by `createdAt`), `author` (the issue-CREATOR login STRING — GitHub `.author.login`, GitLab `.author.username`; absent/unresolved → `""`, never null — added by #492, folded into [INV-138], as the @-mention target for human-notice comments; a single-user login, NOT the group/namespace `REPO_OWNER` names). **`author` is appended to the underlying GitHub `gh issue view --json` field set ONLY when the caller requests it** (exactly like `comments`) — so the argv for the pre-[INV-138] field sets (`title,body,state,labels`, etc.) stays byte-identical and every existing golden-trace / parity stub ships unchanged; the GitLab leaf reads the whole issue object regardless, so it simply projects the extra key. Task not found / read failure → rc≠0 with NO partial output (fail-closed). This is a deliberate SHAPE change (the byte-identical-argv constraint the pre-#396 leaf satisfied is explicitly LIFTED for this verb, mirroring [W1a]'s #371 precedent) — proven by DECISION-level behavior-parity tests (`tests/unit/test-w1b-read-task-parity.sh`) instead of argv golden traces. **Asserted** (shape/fields-subset/fail-closed) by `tests/provider-conformance/run-provider-conformance.sh` (#396, R6); the `author` field additionally by `tests/unit/test-w1b-read-task-contracts.sh` (github), `tests/unit/test-itp-gitlab.sh` TC-WB-041b (gitlab), and `tests/unit/test-issue-mention-login.sh` (both + helper). |
...
| `itp_label_event_ts ISSUE LABEL [MODE]` | `dispatcher-tick.sh` Step-2 TTHW timeline read (`gh api …/issues/<n>/timeline --jq …`, the [INV-70] `labeled_at`) | With MODE omitted, this is the original **observe-only / non-blocking** INV-93 metrics read: echo the FIRST added/labeled timestamp and preserve the non-paginated query byte-for-byte. MODE=`latest-removed` is a separate fail-soft INV-144 control observation: it returns the latest removed/unlabeled event and paginates so an older operator re-arm cannot fall outside GitHub's first timeline page. Either mode returns **empty** if none / on failure / leaf-absent; in removal mode empty means no re-arm cutoff, never an I/O abort. INV-70's metrics-only guarantee applies unchanged to the default TTHW caller, not to INV-144's optional mode. A **focused verb**: backend event vocabulary stays in the leaf and the caller receives a neutral scalar. The GitHub leaf JSON-encodes `LABEL` into the jq string literal. **New verb [m]; migrated #323, optional removal mode #525.** **Asserted** by `tests/unit/test-label-event-ts.sh`, `tests/unit/test-itp-gitlab.sh`, and the fail-soft provider conformance gate. |
...
owns, **21 table rows naming 22 verbs** (one row, `chp_review_threads` /
...
review r8 / #329), the 4 focused single-purpose verbs added by later PRs
(`chp_list_inline_comments` #328, `chp_reply_review_comment` #327,
`chp_count_reviews_by_login` #324, `chp_commit_file` #330), the
PR-diff-size read primitive (`chp_pr_diffstat`, #452), and the reviewed-HEAD
CI-rollup gate read (`chp_ci_rollup`, #489, [INV-134]). **`chp_caps` is a
...
precisely so a future mint does not miscount it as a 23rd verb. The GitHub
reference implementation (`lib-code-host.sh`) therefore defines **23
top-level `chp_*()` functions**: the 22 verb shims (21 table rows) + the 1
non-verb `chp_has_leaf` guard helper. Where a count needs to appear in prose
elsewhere in this repo, prefer citing "the §3.2 table (21 rows / 22 verbs)"
...
| `chp_ci_rollup PR` | (new) — the review wrapper's PASS-chain approval gate (`autonomous-review.sh`, [INV-134], issue #489) | **Reviewed-HEAD CI-rollup gate leaf, SIBLING of `chp_ci_status`, NOT a replacement (issue #489 D1).** `chp_ci_status`'s deliberate SKIPPED→`pending` mapping is correct for its green-corroboration use (`_e2e_ci_green_precheck`) and WRONG for an approval gate — a repo with a permanently-SKIPPED label-gated check (this repo's own self-hosted agent-smoke check) would never read green under that mapping, permanently blocking approval. Stdout is a **JSON object** `{"token":"<green\|pending\|failed\|none>","failed_checks":["<name>",...]}` (NOT a bare token — the shape change from `chp_ci_status` is deliberate, so the wrapper can name the failing check(s) in its finding). Token derived from the per-check state multiset by this decision order: (1) any check ∈ {`FAILURE`,`ERROR`,`CANCELLED`,`TIMED_OUT`} → `failed` (`failed_checks` lists exactly those checks' names); (2) else any check ∈ {`PENDING`,`QUEUED`,`IN_PROGRESS`,`EXPECTED`} or any state not otherwise listed → `pending` (`failed_checks` lists those still-unresolved checks' names too — the wrapper's bounded-wait give-up finding needs them; `none`/`green` are the only tokens that leave it `[]`); (3) else zero checks → `none`; (4) else → `green` (`SUCCESS`, `SKIPPED`, and `NEUTRAL` are ALL non-blocking — an all-SKIPPED set is `green`, not `none`, unlike `chp_ci_status`). Rule 1 beats rule 2 (a FAILURE+PENDING set is `failed`). **Scope**: every check run / status context the code-host API returns for the PR — no required-check allowlist; a path-filtered workflow that creates no API-visible check run is outside the evaluated set by construction. rc≠0 on transport/parse failure with EMPTY stdout (fail-closed, mirrors `chp_ci_status`'s empty-stdout and non-array payload guards). A backend that cannot return check names returns a stable job/pipeline identifier in `failed_checks` instead (the GitLab leaf's job-name fallback). **No `gh` flags and no jq programs cross the seam.** Not capability-gated — every CHP provider MUST implement it (mirrors `chp_ci_status`/`chp_mergeable`'s ungated posture). Asserted (token-set + `failed_checks` contents + fail-closed + SKIPPED-is-green divergence from `chp_ci_status`) by `tests/provider-conformance/run-provider-conformance.sh`. |
...
| `chp_pr_diffstat PR DIMENSIONS-CSV` | the PR-diff-size (over-reach) soft signal (`autonomous-review.sh`, [INV-124], issue #452) | **General READ primitive**, sibling of `chp_pr_view`/`chp_pr_list` — added so the caller's diff-stat read carries zero executable raw `gh pr view --json additions,deletions,changedFiles`. `DIMENSIONS-CSV` is a non-empty subset of `{files,lines}` (comma-separated). Returns a **single normalized JSON object** carrying ONLY the requested key(s) — `files` → `{"changed_files": <int>}`, `lines` → `{"changed_lines": <int>}` — a dimension is OMITTED (never fabricated 0/null) when the provider could not determine it. **Cost differs by provider, not capability — both dimensions are supported on both hosts.** GitHub: `gh pr view --json additions,deletions,changedFiles` answers BOTH dimensions in ONE call regardless of which is requested (zero marginal cost); `changed_lines` is `additions + deletions`. GitLab: `files` reads the base MR view's `.changes_count` (a STRING, populated asynchronously, capped and rendered as the literal `"1000+"` above 1000 — parsed down to the integer `1000`) at zero extra cost (the base MR view is already fetched); `lines` requires a **SEPARATE** GraphQL `diffStatsSummary { additions deletions }` call (`_gl_graphql`, the FIRST GraphQL call site in `chp-gitlab.sh` — GitLab's GraphQL endpoint authenticates via `Authorization: Bearer`, not the REST `PRIVATE-TOKEN` header `_gl_api`/`_gl_http` use, so this is a separate transport primitive in `lib-gitlab-transport.sh`, not a `_gl_api` call; a `GITLAB_TRANSPORT_HOOK`-only installation with no `GITLAB_TOKEN` can still answer this call by additionally defining the optional `_gl_graphql_hook` override — §3.5.1, #452 amendment) — issued **ONLY when `lines` is in `DIMENSIONS-CSV`** (pay-only-if-requested; a `files`-only request never reaches the GraphQL leaf). **Independent failure domains (data-source honesty)**: a GraphQL failure (auth/network/schema error) on the GitLab leaf does NOT suppress a `files` result already read successfully from the base MR view — the leaf omits ONLY the `changed_lines` key on that failure, still returning `{changed_files: N}` at rc 0 when `files` was also requested. Fail-CLOSED (rc≠0, no partial output) only on a HARD failure that prevents answering ANY requested dimension (base MR view fetch failure on GitLab; `gh pr view` failure / non-object JSON on GitHub). Self-guarding shim (leaf-absent → WARN + `return 1`), same posture as `chp_pr_view`/`chp_pr_list`. The caller (`lib-review-diffcap.sh::review_diff_over_reach`) treats an OMITTED key as an unreadable stat — fail-open, never fabricates an over-reach warning from it. **NEW #452** — this is a **prompt-weighting signal, not a hard gate** ([INV-124]): `over_reach` is never read by any verdict-aggregation code path. |
...
| `authorKind` | normalized enum `bot` / `human` / `self`. **GitHub derivation MUST be REST-sourced (#393)**, priority order: `self` FIRST (`user.login` matches a resolved self login in raw OR `[bot]`-stripped form), then `bot ⇔ user.type == "Bot"` (authoritative), else `human`. Normally self is `BOT_LOGIN`. With private opt-in `ITP_REQUIRE_SELF_AUTHOR=1`, GitHub switches to exact-login matching: PAT mode resolves `GET /user` when `BOT_LOGIN` is empty; App mode resolves the exact dev, review, and dispatcher App slugs from configured credentials by signing one JWT per App and calling `GET /app`, and every App-mode self match additionally requires REST `user.type == "Bot"`. Thus neither a human matching an App slug nor an App matching a PAT login is promoted. Installation tokens are never treated as supporting `/user`, and unrelated Apps remain `bot`. GitLab strict mode always resolves the active provider-local `/user`, ignoring any inherited `BOT_LOGIN` that may belong to a different code host, then adds only exact usernames from `TERMINAL_CONTROL_TRUSTED_AUTHORS` for cross-role terminal-control recovery. Any required identity-resolution failure is fail-closed. Ordinary reads retain the established compatibility and remain byte-identical. GitHub's GraphQL comment read strips the `[bot]` suffix and exposes no author type, so `itp_list_comments` remains the authoritative per-issue read. | lets authenticity-sensitive callers reject human and unrelated-bot forgeries while permitting explicitly configured pipeline roles to recover each other's durable markers. The bulk state-read's embedded comments remain unsuitable for authenticity gates. **New field [M5].** |
...
**Override hook (`GITLAB_TRANSPORT_HOOK`)** — if set (conf-declared path), the transport lib sources it at library-init BEFORE any leaf runs. The hook MUST redefine `_gl_http` (same signature/contract); `_gl_api` stays lib-owned so pagination + backoff + fail-closed cannot be lost by a variant. **Trust model**: operator-owned local code, same privileges as `autonomous.conf` — explicitly NOT a sandbox (#414 pillar 3). Preflight fail-loud, latched once per process (mirrors `_AGENT_TOKEN_PAT_WARNED` at `lib-auth.sh:93`): `GITLAB_TOKEN` unset when no hook is armed → rc ≠ 0 with recovery guidance; `GITLAB_TRANSPORT_HOOK` set but path unreadable → rc ≠ 0 naming the path; after sourcing the hook, `_gl_http` must exist and be callable or preflight fails rc ≠ 0. `GITLAB_HOST` default: `gitlab.com`.

**`_gl_graphql <query> [variables-json]`** (#452, [INV-124]) — the GitLab GraphQL endpoint primitive backing `chp_gitlab_pr_diffstat`'s `lines` dimension (`diffStatsSummary`). GitLab's GraphQL endpoint (`https://${GITLAB_HOST}/api/graphql`) does not accept the REST `PRIVATE-TOKEN` header `_gl_http` sends — its default impl authenticates via `Authorization: Bearer ${GITLAB_TOKEN}` instead, so it is a separate primitive from `_gl_http`/`_gl_api`, not a REST call routed through them. Single-shot; no pagination; no `429` retry (unlike `_gl_api`) — the sole caller is a single object-shaped read behind a fail-open soft signal, so a hard one-shot failure is an acceptable outcome. Fail-CLOSED (rc ≠ 0, no stdout) on transport failure / non-2xx / empty body / a populated `errors` array / null `data`; on success echoes the inner `.data` object.

**Optional second hook point, `_gl_graphql_hook`** (#452 amendment, [INV-124]) — `_gl_graphql` ensures the transport preflight has run (sourcing `GITLAB_TRANSPORT_HOOK` if armed; idempotent/latched, so this is safe even if `_gl_graphql` is the very first transport call in the process) and then checks whether the hook additionally defined a function named `_gl_graphql_hook <query> <variables-json>`. If present, `_gl_graphql` delegates to it, instead of running the default Bearer-token impl. This closes the gap the original #452 design left: a `GITLAB_TRANSPORT_HOOK`-only installation with no `GITLAB_TOKEN` (an SSO gateway, cookie-sync tooling, a forked CLI) could authenticate `_gl_http`/`_gl_api` calls through its own transport but had NO way to answer the GraphQL endpoint, so the `lines` dimension always degraded to unreadable even though the pipeline advertises "both dimensions supported on both hosts." `_gl_graphql_hook` is strictly ADDITIVE and OPTIONAL — a hook that redefines `_gl_http` only (the pre-#452-amendment contract) continues to work unchanged; `_gl_graphql` simply falls through to its default Bearer-token impl (still requires `GITLAB_TOKEN` in that case, and the `lines` dimension degrades to unreadable exactly as before this amendment when the hook does not opt in and no token is configured). `_gl_graphql_hook`'s absence is NOT preflight-validated (unlike `_gl_http`, which the hook MUST redefine) — it is a valid, expected state. **The hook's output is NOT trusted verbatim** (round-2/round-3 review findings): `_gl_graphql` captures the hook's stdout rather than streaming it, so a nonzero hook rc can never leak partial output, and a rc-0 result is validated with `jq -e -cs 'length == 1 and (.[0] | type == "object")'` (slurp mode, requiring EXACTLY ONE top-level JSON object) before being handed to the caller. Slurping is required, not a bare `jq -e 'type == "object"'` — the latter only checks the LAST value parsed from a multi-document stream and would wrongly pass a hook that double-prints (e.g. concatenating a stray error/log object with the real data object). A hook returning malformed JSON, a top-level `null`, or a multi-document stream all fail CLOSED, keeping the fail-closed shape contract uniform with the default impl's own `.data != null` guard.

The two-layer contract is normatively pinned by [INV-116](invariants.md#inv-116-the-gitlab-transport-is-a-two-layer-choke-point-_gl_http-request-primitive--_gl_api-public-function-pagination--backoff--fail-closed-live-in-_gl_api-the-override-hook-may-redefine-_gl_http-only). A future amendment (spec + INV entry + code) is the ONLY way to change it — a W-B slice cannot redesign it silently. `_gl_graphql`/`_gl_graphql_hook` is a SIBLING primitive alongside this frozen contract, not a modification of it — INV-116's `_gl_http`/`_gl_api` two-layer rule is unchanged.
...
assignees=1                  # itp_list_by_state/itp_list_forbidden_combos normalize an assignees field (#435)
...
### 4.1 ITP capability matrix (10 keys)
...
| `assignees` | ✓ | ✓ | — (not yet evaluated) | **New #435 (ISSUE_FILTER seam PR-A).** `itp_list_by_state`/`itp_list_forbidden_combos` normalize an `assignees` field (array of login strings). Consumed by the follow-up `ISSUE_FILTER` feature's assignee-atom capability gate (fail-closed: a provider that hasn't declared `assignees=1` cannot be sliced by assignee) — no caller branches on it yet in this PR |
...
> **Transport contract shipped in #416 W-A** — see [§3.5.1 GitLab transport contract](#351-gitlab-transport-contract-transport--the-two-layer-choke-point). Every GitLab leaf (W-B / W-C, upcoming) reaches the network through `_gl_api` (public function; owns pagination + `429`/`Retry-After` backoff + `§3.5` fail-CLOSED discipline) → `_gl_http` (single-request primitive; the OUT-OF-TREE hook point). Instance-specific auth (SSO gateways, cookie-sync tooling, forked CLIs) is served by the operator-owned `GITLAB_TRANSPORT_HOOK` (MUST redefine `_gl_http`; MAY additionally define `_gl_graphql_hook` to also cover the GraphQL endpoint that `chp_gitlab_pr_diffstat`'s `lines` dimension uses — #452 amendment, [INV-124]). PAT is the reference default via `GITLAB_TOKEN` against `${GITLAB_HOST}` (default `gitlab.com`). GitLab has no GitHub-App equivalent (see below), so the agent's `GITLAB_TOKEN` posture degrades to convention — parallel to the existing GH_AUTH_MODE=token WARN (`_AGENT_GITLAB_TOKEN_PAT_WARNED` latch in `lib-auth.sh`, #416 R2). `LEAVES NEVER call curl or _gl_http directly` — enforced by the [INV-91] cutover-guard extension (#416 R4).
...
GitLab policy states intentionally remain `UNKNOWN`; they are not structural
conflict evidence. [INV-149](invariants.md#inv-149-a-consumed-same-head-non-substantive-retry-marker-cannot-make-a-historical-mergeable-unknown-observation-terminal-without-a-current-head-pinned-provider-read)
prevents that historical token from becoming a later terminal same-HEAD stall
without a current bounded provider poll. A terminal persistent `UNKNOWN`
requires all poll reads to succeed; an undecided poll containing a transport
failure returns operational defer instead. This does not weaken the wrapper's
independent CI, approval, discussion, or final mergeability gates.
...
(`tests/unit/test-provider-caps-branches.sh`). It splits the 14 caps (10 ITP + 4
...
- **WAIVED (6)** — the caps whose caller branch is not yet wired (it lands with the
  GitLab/Asana PRs, or — for `assignees` (#435) — the follow-up ISSUE_FILTER PR-B,
  §4.3; the degraded fixture `.sh` are empty scaffolds, so there
...
The headline prints `exercised=8 waived=6 total=14` and asserts the split equals
the full matrix, and a tripwire self-test proves the branch-detector is not a
no-op grep. (Exercising all 14 is not possible on a GitHub-only HEAD — 6 branches
...
| `handle_completed_session_routing` (`lib-dispatch.sh`) | `itp_post_comment` + `itp_transition_state` + `chp_find_pr_for_issue` (+ dispatch) | **(b) entangled multi-op orchestrator** | **leaf I/O migrated #283/#282; golden-trace gate landed #285.** Per-arm cut: `none`/default → `itp_list_comments` (dedup) + `itp_post_comment` (INV-12-completed handoff); `passed` → no host op (race no-op); `failed-non-substantive` → `itp_list_comments` through `count_review_aware_flips`, then `itp_post_comment` (review-aware-flip) + `label_swap pending-dev pending-review`, at REVIEW_RETRY_LIMIT `itp_post_comment` + `mark_stalled`, or operational defer on unreadable/malformed accounting; `failed-substantive` → `fetch_pr_for_issue "$issue_num" "number,headRefOid,body"` (→ `chp_find_pr_for_issue`, the #148 body-inclusion anchor) then Branch A (bot-unfixable) / B (no-progress) `itp_list_comments`+`itp_post_comment`+`mark_stalled`, or Branch C `itp_list_comments`+`itp_post_comment`(INV-35-fresh-dev)+`label_swap pending-dev in-progress`+`post_dispatch_token`+`dispatch dev-new`+`itp_post_comment` (the [INV-85] `no-progress-substantive-attempt:<head>` HTML marker, #274 anchor). **Caller-side (NOT a verb):** `classify_recent_review_verdict`, `count_review_aware_flips` (ordinary markers plus INV-149 reservation/completion union keyed by session+HEAD+ordinal), `last_reviewed_head`, `dev_report_bot_unfixable`, the [INV-35]/[INV-85] routing decision, the `: > $log` truncate, `post_dispatch_token`, `dispatch dev-new`. NO caps gate inside the function. |
...
| `itp_list_forbidden_combos STATE LABELS_AND_CSV LIMIT` | same enumeration; jq combo filter in-leaf (native `not[labels]=X` is insufficient for intersection-of-incompatible-sets) | Output fields: `number,labels,assignees` (widened from `number,labels` #435, ISSUE_FILTER seam PR-A — unconditional, this verb takes no `FIELDS_CSV`). [INV-25]. Fail-CLOSED. |
...
| `itp_list_comments ISSUE` | `GET /projects/${GITLAB_PROJECT}/issues/${ISSUE}/notes?sort=asc&order_by=created_at` (paginated) | [INV-90] normalized array. **System notes (`.system==true`) FILTERED OUT** (audit-trail events, not comments — matches CHP-side discipline). `id`=native numeric; `author`=`.author.username`; `authorKind`: `self` iff username matches the resolved self login, else `bot` iff username matches `^(project\|group)_\d+_bot(_[a-z0-9]+)?$`, else `human`. Normally self is `BOT_LOGIN`; with `ITP_REQUIRE_SELF_AUTHOR=1`, always resolve provider-local `GET /user` and fail closed if unavailable, even when an inherited nonempty `BOT_LOGIN` belongs to another code host. That private opt-in also promotes only exact comma-separated usernames from `TERMINAL_CONTROL_TRUSTED_AUTHORS`; ordinary reads ignore the list. Sort ascending by `createdAt`, `id` tie-break. |
...
| `itp_label_event_ts ISSUE LABEL [MODE]` | `GET /projects/${GITLAB_PROJECT}/issues/${ISSUE}/resource_label_events` (paginated) | Endpoint doesn't accept a label filter, so jq filters locally. Default mode selects `.action == "add"` and emits the FIRST matching timestamp; `latest-removed` selects `.action == "remove"` and emits the latest. LABEL is bound via `jq --arg lbl`. **Fail-SOFT**. |
