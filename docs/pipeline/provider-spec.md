...
| `itp_list_comments ISSUE` | `GET /projects/${GITLAB_PROJECT}/issues/${ISSUE}/notes?sort=asc&order_by=created_at` (paginated) | [INV-90] normalized array. **System notes (`.system==true`) FILTERED OUT** (audit-trail events, not comments — matches CHP-side discipline). `id`=native numeric; `author`=`.author.username`; `authorKind`: `self` iff username matches the resolved self login, else `bot` iff username matches `^(project\|group)_\d+_bot(_[a-z0-9]+)?$`, else `human`. Normally self is `BOT_LOGIN`; with `ITP_REQUIRE_SELF_AUTHOR=1` and no `BOT_LOGIN`, resolve `GET /user` and fail closed if unavailable. That private opt-in also promotes only exact comma-separated usernames from `TERMINAL_CONTROL_TRUSTED_AUTHORS`; ordinary reads ignore the list. Sort ascending by `createdAt`, `id` tie-break. |
