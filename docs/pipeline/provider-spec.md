...
owns, **20 table rows naming 21 verbs** (one row, `chp_review_threads` /
...
`chp_count_reviews_by_login` #324, `chp_commit_file` #330), and the
PR-diff-size read primitive (`chp_pr_diffstat`, #452). **`chp_caps` is a
...
precisely so a future mint does not miscount it as a 22nd verb. The GitHub
reference implementation (`lib-code-host.sh`) therefore defines **22
top-level `chp_*()` functions**: the 21 verb shims (20 table rows) + the 1
non-verb `chp_has_leaf` guard helper. Where a count needs to appear in prose
elsewhere in this repo, prefer citing "the §3.2 table (20 rows / 21 verbs)"
