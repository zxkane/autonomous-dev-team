...
4. `resume_agent SESSION_ID PROMPT MODEL`. PROMPT is fed via stdin ([INV-33](invariants.md#inv-33-agent-prompt-is-fed-via-stdin-never-as-a-single-argv-element)). For claude: `printf '%s' "$PROMPT" | claude --resume ID --permission-mode auto -p --output-format json`. The `--name` flag is omitted on resume (claude doesn't update display name on resume).
