...
- **Idempotent against label state** ([INV-08](invariants.md#inv-08-wrapper-exit-trap-is-idempotent-against-label-state)): the trap uses `--remove-label X --add-label Y` in single calls. If the dispatcher's Step 5a already moved the issue to `pending-review`, the trap's `−in-progress +pending-review` is a no-op for the present `pending-review` and a successful remove for the (already absent) `in-progress`. Final state is `pending-review` either way.
