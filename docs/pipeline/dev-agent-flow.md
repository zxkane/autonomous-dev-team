...
│                #     WRAPPER_START, WRAPPER_FINGERPRINT, GUARDIAN_IDENTITY,
│                #     GUARDIAN_PID, CREATED_EPOCH, STATE
├── pgids        # append-only: "<pgid> <role> <epoch> <identity>" per spawn
├── pgids.lock   # serializes complete four-field pgids appends
├── pgids.closed # strict delayed GC has closed further registration
...
Installation is atomic: the KV + sidecars are fully written inside `lanes/.pending-<id_fs>/`, then the directory is renamed into its final `lanes/<id_fs>/` path (`mv -T`) — a half-written lane directory under its final name is never observable. `ADT_STATE_ROOT` deliberately ignores `XDG_STATE_HOME` (unlike `lib-metrics.sh`/`lib-run-artifacts.sh`, which DO honor it). An explicit non-empty `ADT_STATE_ROOT` wins; otherwise the shared `lib-state-root.sh` resolver reads the host-user pointer `$HOME/.local/state/adt-state-root`, which `install-gc-timer.sh` atomically writes with the timer's validated absolute root. A missing pointer defaults to `$HOME/.local/state`; malformed, unreadable, or non-regular pointer state warns and falls back there. `lib-lane.sh`, local dispatcher marker reads through `lib-dispatch.sh`, and the embedded remote-SSM liveness command all use this resolver, keeping wrappers, the periodic timer, opportunistic GC, and the box-wide rollback file on one canonical anchor even when their environments differ.

On success, `ADT_LANE_ID`/`ADT_LANE_DIR` are exported into the wrapper's own shell, so the token daemon, the heartbeat subshell, and the agent CLI all inherit the tag via ordinary environment inheritance — no per-spawn-site plumbing needed for the tag itself. `_run_with_timeout` (`lib-agent.sh`) additionally appends every spawn's PGID to `${ADT_LANE_DIR}/pgids` via `lane_record_pgid`, reading the diagnostic role tag from `ADT_LANE_ROLE` (default `agent`). The append is serialized on `pgids.lock` and records `<pgid> <role> <epoch> <identity>`. The compact, whitespace-free Linux identity is `v2-linux:<boot-id>:<starttime-ticks>`; the BSD/macOS diagnostic fallback is `v1-bsd:<recorded-ppid>:<sha256(comm+recorded-ppid+lstart)>`. Only the boot-bound Linux v2 form authorizes delayed signaling.

The identity is load-bearing only for a delayed periodic-GC decision, where `lane_kill ... require-identity` must first own `reap.lock`, closes registration by creating `pgids.closed` while holding `pgids.lock`, verifies the whole live PGID set before any TERM, waits one shared grace interval, then verifies every survivor before starting KILL. It revalidates each target again immediately before that target's TERM/KILL. A phase-preflight refusal sends nothing in that phase; a later per-target refusal stops the remaining signals but cannot undo earlier signals already sent in the same phase. Only an exact recorded `BACKEND=pgid` is accepted; missing, unknown, and `systemd-scope` values refuse pending #522. A live legacy three-field record, pre-boot-ID `v1-linux`, BSD v1, `-`, malformed identity, unreadable live leader, or mismatch is likewise not kill authorization and the delayed reap fails toward leak. Pass 2/3 capture each candidate PID identity before env/argv/cwd classification, pass it and the PGID-leader identity explicitly through the guard and escalation helpers, and revalidate before signaling; `proc_identity_authorizes_signal` is the single durable authorization predicate. Wrapper cleanup, guardian EOF cleanup, and other immediate ownership paths omit the strict policy and retain their existing best-effort behavior; this keeps graceful/crash cleanup compatible with lanes minted before the schema widening.
...
Immediately after `lane_install` succeeds (and before any other background work, including the auth setup below): `mkfifo "${ADT_LANE_DIR}/guard.fifo"` → `exec {ADT_GUARD_FD}<>"${ADT_LANE_DIR}/guard.fifo"` (the wrapper's own WRITE end, opened `<>` so it never blocks) → a `setsid`-hard-prerequisite check → `setsid bash lib-guardian.sh --lane-dir "$ADT_LANE_DIR" &` (its own spawn line closes its inherited copy of `ADT_GUARD_FD` first, exec'ing the guardian only after) → capture `proc_identity $!` → `lane_set GUARDIAN_IDENTITY ...` → `lane_set GUARDIAN_PID $!`. Identity is published first so concurrent GC can observe either the original `GUARDIAN_PID=-` or the complete identity/PID pair, never a new live PID paired with stale identity.
...
Delayed GC validates a live `GUARDIAN_PID` against `GUARDIAN_IDENTITY` immediately before TERM and again before KILL. A proven initial mismatch is treated as PID reuse and is never signaled; because it proves the recorded guardian is gone, stale metadata may still be collected. A missing, legacy, malformed, unreadable, or signal-time-changed identity is unverifiable, so delayed GC sends no further signal and preserves the lane directory (fail toward leak). The wrapper-to-guardian handshake and the guardian's immediate EOF-triggered reap remain best-effort and do not depend on that delayed-GC identity check.
