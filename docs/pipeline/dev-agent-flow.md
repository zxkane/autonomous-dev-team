...
- **Atomically acquiring the (issue, mode) start slot** ([INV-103](invariants.md#inv-103-acquire_pid_guard-acquires-the-per-issue-mode-start-slot-atomically--no-check-then-write-toctou-window), #360/302a): an `mkdir` on `${pid_file}.lockdir` — atomic at the kernel level, so two near-simultaneous callers can never both observe "no live peer" — serializes the read-existing-PID + `kill -0` probe + write below against every other concurrent caller on the same path. A caller that loses the `mkdir` race retries briefly, reclaims the lock if it looks stale (its holder likely crashed mid-hold), and otherwise exits 0 quickly, logging exactly one line and writing nothing.
...
Read-side consumers — `pid_alive` / `get_pid` (`lib-dispatch.sh`) and `liveness-check-remote-aws-ssm.sh` — are unaffected: they read the exact same path with the exact same content contract (winner's PID, numeric, `kill -0`-able) before and after this change. The lock directory exists only transiently around the acquire and never appears in any read-side check.
