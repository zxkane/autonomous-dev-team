...
| macOS non-Chrome hardened-runtime-suppressed descendants (codesigned binaries that re-setsid; env unreadable AND no argv marker) | invisible on the portable path; only closed by INV-113 scopes (Linux) or `sysctl kern.procargs2` where argv still carries the tag; flag-only Pass-4-style predicate (env empty ∧ ppid==1 ∧ age > floor ∧ pgid outside all live sets ∧ ancestor-tagged) is retained as a monitored option | ✅ stated in §7 & INV-110 (F5 completeness) |
