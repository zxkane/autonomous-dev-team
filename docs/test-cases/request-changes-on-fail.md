...
| TC-RC-SRC-02 | the substantive FAIL branch (`failed-substantive`) calls the helper | helper invoked on the `pending-dev` substantive route |
| TC-RC-SRC-03 | the merge-conflict (`block-substantive`) route calls the helper | helper invoked before its `−reviewing +pending-dev` |
| TC-RC-SRC-04 | the mergeable-UNKNOWN (`block-nonsubstantive`) route does NOT call the helper | no helper call in that branch |
