...
| TC-TIMEOUTGUARD-029 | Boundary tie: the session LEADER exits naturally at essentially the exact instant `AGENT_TIMEOUT` elapses — BEFORE the watchdog (already past its deadline sleep) has written ANY marker — while a TERM-ignoring descendant is still alive | `_run_with_timeout` must NOT infer "watchdog never fired" from an empty marker alone — it also checks the process GROUP's own liveness before cancelling; a still-alive group defers to the watchdog's pending escalation. Reported rc normalizes to `137` (not the leader's own natural `0`) and the descendant does not survive past the function's return (PR #469 review round-5 [P1]) |
| TC-TIMEOUTGUARD-032 | `mktemp` fails at watchdog-spawn time (e.g. read-only/exhausted `TMPDIR`), so `_wd_result_file` is empty for the ENTIRE call — no marker file is EVER possible, unlike TC-TIMEOUTGUARD-025..029 which all still had a working marker mechanism. Exercised via the TC-025 shape (a plain TERM-obeying LEADER, no descendant, real 30s grace default) rather than TC-026/027's shrunk-grace shape — a shrunk grace combined with a TERM-obeying leader is itself a `kill -0`-right-after-`kill -TERM` timing race independent of this fix — so a real fix is distinguishable from a coincidence | `_run_with_timeout` still normalizes rc to `124` (not the leader's own natural `0`) via the watchdog's own exit status as a filesystem-independent fallback result channel, captured uniformly whether reconciliation takes the block-for-escalation path or the cancel path, with the watchdog immune (via `trap "" TERM` on itself, installed once committed post-deadline) to having its own committed exit code truncated by the caller's cancel signal racing its in-flight TERM — proving rc normalization does not depend on the marker file mechanism working (PR #469 review round-6 [P1], plus a follow-up fix for a second race the same review round's fix initially missed) |
