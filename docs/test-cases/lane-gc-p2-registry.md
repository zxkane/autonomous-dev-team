...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-LGC2-020 | A test child process spawned (under `env -i`, so the probe is immune to ambient env pollution — see regression note below) after `export ADT_LANE_ID`/`ADT_LANE_DIR` in the calling shell | `/proc/<child-pid>/environ` contains `ADT_LANE_ID=<exact value>` AND `ADT_LANE_DIR=<exact value>` verbatim (verified via `tr '\0' '\n'` — the `env_of` shim's own read path; asserted by exact-value match, never a line COUNT) |
| TC-LGC2-020-regression | **Regression (codex review [P1], pre-fix reproduced live):** this test suite itself runs inside a review-agent context that already exports `ADT_LANE_ROLE` (this PR's own fan-out/smoke/E2E tagging) alongside `ADT_LANE_ID`/`ADT_LANE_DIR` — a bare `grep -c "^ADT_LANE_"` count then reads 3, not 2, and fails a hardcoded `-eq 2` check. Run TC-LGC2-020 with `ADT_LANE_ID`/`ADT_LANE_DIR`/`ADT_LANE_ROLE` all pre-set in the test runner's OWN environment (simulating a review-agent fan-out ambient env) | still passes — `env -i` isolates the probe subshell from ANY ambient `ADT_LANE_*` var, and the assertion checks the two named values rather than counting lines |
