# Test cases: structured AC-coverage artifact (INV-47, #183)
...
| TC-AC-SRC-02 | `build_review_prompt` prefers the structured map when the sidecar is non-empty | grep matches a structured-map branch keyed on the sidecar |
| TC-AC-SRC-03 | `build_review_prompt` still emits the free-form `## E2E Evidence` block when no sidecar | the free-form block remains reachable (back-compat) |
...
| TC-AC-DOC-02 | `invariants.md` has an `INV-47` entry | grep matches |
