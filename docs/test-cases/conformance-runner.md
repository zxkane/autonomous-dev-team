...
| TC-CONFORMANCE-037 | Codex stub does NOT hang on a non-EOF stdin (PR #244 [P1], codex review `1c29ba19`): the codex review path carries the prompt as an argv positional and pipes no stdin, but the stub unconditionally `cat > .stdin`. A local TTY run would block the stub forever (rc 124); the dispatch now feeds `</dev/null`. 037a drives the real codex run with a fifo+open-writer (TTY proxy) → exit 0 (124 pre-fix); 037b source-of-truth (`</dev/null` on the dispatch). | exit 0 under a blocking stdin; dispatch redirects `</dev/null` |
| TC-CONFORMANCE-038 | Codex review control env is reset (PR #244 [P1], codex review `1c29ba19`): `_run_codex_review` reads `CODEX_REVIEW_MAX_RERUNS` (default 3) / `AGENT_REVIEW_TIMEOUT` (default 1h) at call time, so an inherited value changes a codex fixture's runtime. An inherited `CODEX_REVIEW_MAX_RERUNS=100000` makes a transient fixture re-run 100000× → hang; the runner resets both to defaults before `input.env`. 038a the leak does NOT contaminate (exit 0, 124 pre-fix); 038b source-of-truth (reset to defaults). | exit 0 under `CODEX_REVIEW_MAX_RERUNS=100000`; runner resets the controls |
