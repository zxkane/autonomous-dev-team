...
| TC-CONFORMANCE-025 | Bad `stdinSha256` (not 64-hex) → schema reject (loud). | FAIL schema-invalid |
...
| TC-CONFORMANCE-027 | INV-34 stdin channel: the stub records the bytes it reads on stdin; the runner asserts the prompt actually reached the stub over that channel. (No byte-compare against `command.stdinSha256` — the runner feeds a live-nonce smoke prompt, not the recorded prompt, so a fixed recorded hash could never match; the digest is schema-validated as 64-hex only.) | stub read nothing → `FAIL stdin-not-fed`; loud, never silently skipped |
...
| TC-CONFORMANCE-041 | The two load-bearing rc mappings are pinned: `124/137 + no verdict ⇒ timeout-veto`, `0 + no verdict ⇒ drop`. | both present as fixtures, both PASS |
