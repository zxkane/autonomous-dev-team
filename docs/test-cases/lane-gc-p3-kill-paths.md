...
| TC-LGC3-005 | **(round-8 [P1])** `lane_kill` running in a caller whose OWN process group is group-SIGKILLed 1s into a 4s grace (the kill_stale_wrapper-escalates-against-a-cleaning-wrapper shape); a TERM-resistant pgid is recorded | the setsid-isolated escalator survives the caller-group SIGKILL and still delivers the follow-up KILL — target gone after grace+settle. Proven to FAIL pre-fix (bare backgrounded escalator died with the caller; target leaked indefinitely) |
...
| TC-LGC3-024c | pgrep-fallback sweep grace: TERM pass → up-to-5s liveness poll (early-exit when both collected sets are empty) → KILL pass | grace parity with the PID-file path (the issue pins kill_stale_wrapper at 5s; the sweep previously slept a flat 1s) |
