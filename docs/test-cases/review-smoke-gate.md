...
| TC-REVIEW-SMOKE-044 | the smoke resolves each member's model via `_resolve_review_agent_model` and applies the INV-38/INV-42 launcher treatment — the SAME resolution the fan-out uses (assert the smoke loop calls the resolver + the non-claude launcher-neutralize) |
