...
| TC-TOKEN-SPLIT-031 | prefix points `GH_TOKEN_FILE` at the SCOPED file (refresh-aware, not unset) + unsets `GITHUB_PERSONAL_ACCESS_TOKEN` AND `GH_USER_PAT` | `GH_TOKEN_FILE=<AGENT_GH_TOKEN_FILE>` present, no `-u GH_TOKEN_FILE`; `-u GITHUB_PERSONAL_ACCESS_TOKEN` present; `-u GH_USER_PAT` present |
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-TOKEN-SPLIT-040 | `_run_with_timeout` runs a stub "agent" that dumps `env`; scoped token armed | dump shows `GH_TOKEN`=scoped, `GH_TOKEN_FILE`=scoped file (not the wrapper's), NO `GITHUB_PERSONAL_ACCESS_TOKEN`, NO `GH_USER_PAT` (scrubbed), the wrapper `GH_WRAPPER_DIR` NOT on PATH but the agent-own shim dir IS |
