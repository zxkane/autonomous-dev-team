...
| ID | Scenario | Expected |
|---|---|---|
| TC-SPEC-GATE-030 | Every label literal written by the seven pipeline files (`label_swap` and direct `itp_transition_state` args plus `--add/--remove-label` literals) appears as a `state` or in an `actions[]` of `transitions.json` | checker exit 0 (C.1) |
...
| TC-SPEC-GATE-034 | No variable-valued label write remains in the seven pipeline files after `label_swap` and `hygiene_strip_residual_labels` delegated to `itp_transition_state` | `variable_write_allowlist.sites` is empty; checker emits neither allowlist INFO nor P1.1 failure; real repo passes |
