...
| TC-HCGT-010b | #148 + W1c1 anchor at the REAL verb boundary | the test runs the LIVE chain `fetch_pr_for_issue → resolve_pr_for_issue → chp_find_pr_for_issue` (only `fetch`/`chp` are recorded; `resolve` is real). Under W1c1 (#397) the caller passes its FIELDS-CSV verbatim (the leaf owns the resolver-key union); the recorded `chp_find_pr_for_issue` FIELDS-CSV is the orchestrator's `number,headRefOid,body` verbatim, which MUST still contain `body` (#148 anchor). NO `-q` crosses the seam under the W1c1 abstract contract. A regression dropping `body` from the orchestrator's FIELDS-CSV is caught HERE, not hidden behind a hand-rolled mock forward (#285 review m3) |
