...
| `skills/autonomous-review/SKILL.md` | 12 | 13 | Scope-noted at top | Every `gh pr …` in this skill (the INV-52 rules about the wrapper owning `--approve` / `--request-changes` / `gh pr merge`, the merge-conflict mergeable check, the PR-diff / CI-status reads, the issue-comments read) is a wrapper-side or agent-facing GitHub-lane form. One top-of-file scope note declares that all `gh` examples are placeholders for the corresponding `chp_*` seams on GitLab (`chp_approve_mr` / `chp_merge` / `chp_mergeable` / `chp_ci_status` / `chp_pr_view`) and that the INV-52 / INV-44 rules apply verbatim across lanes. |
