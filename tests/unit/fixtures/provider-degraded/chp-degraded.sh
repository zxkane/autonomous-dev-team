...
# _chp_degraded_list_fields_ok <fields-csv> — reject `author` (pr_view-only,
# issue #495) and any other name outside the 13-member W1c1 list-side
# vocabulary `_chp_degraded_build_projection` knows how to project. Mirrors
...
_chp_degraded_list_fields_ok() {
  local fields="$1" _CHP_DEG_LIST_VOCAB="number,state,title,body,createdAt,updatedAt,mergedAt,headRefName,headRefOid,reviewDecision,mergeable,closingIssueNumbers,reviews"
  local IFS_SAVE=$IFS; IFS=','
  # shellcheck disable=SC2206
...
chp_degraded_find_pr_for_issue() {
  local fields="${2:-}"
  [ -n "$fields" ] || return 2
  _chp_degraded_list_fields_ok "$fields" || return 2
  local raw
...
chp_degraded_pr_view() {
  ...
  local _CHP_DEG_PRV_VOCAB="number,state,title,body,createdAt,updatedAt,mergedAt,headRefName,headRefOid,reviewDecision,mergeable,closingIssueNumbers,comments,reviews,author"
